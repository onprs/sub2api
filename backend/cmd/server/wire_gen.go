...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	apiKeyHandler := handler.NewAPIKeyHandler(apiKeyService, settingService, gatewayService, pricingService)
	...
	ticketRepository := repository.NewTicketRepository(db, configConfig)
	ticketService := service.NewConfiguredTicketService(ticketRepository, configConfig)
	ticketAttachmentRepository := repository.NewTicketAttachmentRepository(db)
	ticketAttachmentS3StoreFactory := repository.NewTicketAttachmentS3StoreFactory()
	ticketAttachmentStoreRegistry, err := repository.NewTicketAttachmentStoreRegistry(configConfig, ticketAttachmentS3StoreFactory)
	if err != nil {
		return nil, err
	}
	ticketStorageSettingsService := service.NewTicketStorageSettingsService(settingRepository, ticketAttachmentRepository, secretEncryptor, ticketAttachmentStoreRegistry, configConfig)
	ticketAttachmentService := service.NewTicketAttachmentService(ticketAttachmentRepository, ticketStorageSettingsService, configConfig)
	ticketHandler := handler.NewTicketHandler(ticketService, ticketAttachmentService)
	channelMonitorRepository := repository.NewChannelMonitorRepository(client, db)
	channelMonitorService, err := service.ProvideChannelMonitorService(channelMonitorRepository, secretEncryptor, groupRepository, gatewayService)
	if err != nil {
		return nil, err
	}
	...
	clinePassClient := service.NewClinePassClient(httpUpstream, configConfig, tlsFingerprintProfileService)
	openRouterClient := service.NewOpenRouterClient(httpUpstream, configConfig, tlsFingerprintProfileService)
	commandCodeClient := service.NewCommandCodeClient(httpUpstream, configConfig, tlsFingerprintProfileService)
	accountUsageService := service.ProvideAccountUsageService(accountRepository, usageLogRepository, claudeUsageFetcher, geminiQuotaService, antigravityQuotaFetcher, grokQuotaFetcher, grokQuotaService, openAIQuotaService, usageCache, identityCache, tlsFingerprintProfileService, clinePassClient, openRouterClient, commandCodeClient, httpUpstream, configConfig, openAIGatewayService)
	accountTestService := service.ProvideAccountTestService(accountRepository, geminiTokenProvider, claudeTokenProvider, grokTokenProvider, antigravityGatewayService, httpUpstream, configConfig, tlsFingerprintProfileService, clinePassClient, openRouterClient, commandCodeClient, openAIGatewayService)
	...
	adminTicketHandler := admin.NewTicketHandler(ticketService, ticketAttachmentService, ticketStorageSettingsService)
	...
	gatewayHandler := handler.ProvideGatewayHandler(gatewayService, openAIGatewayService, geminiMessagesCompatService, antigravityGatewayService, userService, concurrencyService, billingCacheService, billingEligibilityService, usageService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, userMessageQueueService, configConfig, settingService, coordinator)
	openAIGatewayHandler := handler.ProvideOpenAIGatewayHandler(openAIGatewayService, gatewayService, concurrencyService, billingCacheService, billingEligibilityService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, opsService, grokQuotaService, configConfig, coordinator)
	...
	commandCodeGatewayService := service.NewCommandCodeGatewayService(commandCodeClient, configConfig, rateLimitService)
	openCodeGoGatewayHandler := handler.ProvideOpenCodeGoGatewayHandler(openCodeGoGatewayService, clinePassGatewayService, openRouterGatewayService, commandCodeGatewayService, gatewayService, concurrencyService, billingCacheService, billingEligibilityService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, configConfig, coordinator)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, ticketHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, openCodeGoGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, asyncImageHandler, batchImageHandler, accountObserverHandler, idempotencyCoordinator, idempotencyCleanupService, ticketMaintenanceService)
	...
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, auditLogMiddleware, stepUpAuthMiddleware, apiKeyService, subscriptionService, opsService, settingService, compositeRouteResolver, redisClient, channelMonitorService, channelMonitorRunner)
	...
	accountExpiryService := service.ProvideAccountExpiryService(accountRepository, accountUsageService)
