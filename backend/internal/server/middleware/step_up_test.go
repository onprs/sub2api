...
func (s stubStepUpSettingReader) GetStepUpEnabled(ctx context.Context) (bool, error) {
...
func TestEnforceStepUpFailsClosedOnSettingError(t *testing.T) {
	...
	ok := enforceStepUp(
		...
		stubStepUpSettingReader{err: errors.New("settings database unavailable")},
	)

	require.False(t, ok)
	require.Equal(t, http.StatusServiceUnavailable, rec.Code)
	require.Contains(t, rec.Body.String(), "STEP_UP_SETTING_UNAVAILABLE")
}
