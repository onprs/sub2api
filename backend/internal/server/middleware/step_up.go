...
type stepUpSettingReader interface {
	GetStepUpEnabled(ctx context.Context) (bool, error)
}
...
func enforceStepUp(c *gin.Context, grantChecker stepUpGrantChecker, userReader stepUpUserReader, settings stepUpSettingReader) bool {
	...
	if settings != nil {
		enabled, err := settings.GetStepUpEnabled(c.Request.Context())
		if err != nil {
			AbortWithError(c, 503, "STEP_UP_SETTING_UNAVAILABLE", "Step-up security setting is unavailable")
			return false
		}
		if !enabled {
			return true
