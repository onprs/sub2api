...
// dynamic group selection, but before the composite rewrite (gateway.go +
// rootRoute helper).
func TestGatewayRoutesGroupModelAllowlistMountedOnEveryGatewayRoute(t *testing.T) {
	...
	// rootRoute helper：动态分组选择之后、compositeTarget 之前。
	rootHelper := regexp.MustCompile(regexp.QuoteMeta(`r.Handle(method, path, limit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), routeAPIKeyGroup, groupModelAllowlist, compositeTarget, requireGroupAnthropic, handler)`))
	require.Regexp(t, rootHelper, source,
		"root alias helper must place the allowlist after dynamic group selection and before compositeTarget")
	...
	chains := []struct {
		...
		{group: "gateway", auth: "gin.HandlerFunc(apiKeyAuth)", routing: "gateway.Use(routeAPIKeyGroup)", marker: "gateway.Use(groupModelAllowlist)", composite: "gateway.Use(compositeTarget)"},
		{group: "gemini", auth: "middleware.APIKeyAuthWithSubscriptionGoogle(apiKeyService, subscriptionService, cfg)", routing: "gemini.Use(routeAPIKeyGroupGoogle)", marker: "gemini.Use(groupModelAllowlist)", composite: "gemini.Use(compositeGeminiTarget)"},
		{group: "antigravityV1", auth: "gin.HandlerFunc(apiKeyAuth)", routing: "antigravityV1.Use(routeAPIKeyGroup)", marker: "antigravityV1.Use(groupModelAllowlist)", composite: "antigravityV1.Use(requireGroupAnthropic)"},
		{group: "antigravityV1Beta", auth: "middleware.APIKeyAuthWithSubscriptionGoogle(apiKeyService, subscriptionService, cfg)", routing: "antigravityV1Beta.Use(routeAPIKeyGroupGoogle)", marker: "antigravityV1Beta.Use(groupModelAllowlist)", composite: "antigravityV1Beta.Use(requireGroupGoogle)"},
	}
	for _, chain := range chains {
		re := regexp.MustCompile(
			regexp.QuoteMeta(chain.group+".Use("+chain.auth) +
				`[\s\S]{0,400}?` + regexp.QuoteMeta(chain.routing) +
				`[\s\S]{0,400}?` + regexp.QuoteMeta(chain.marker) +
				`[\s\S]{0,400}?` + regexp.QuoteMeta(chain.composite))
		require.Regexp(t, re, source,
			"%s chain must mount groupModelAllowlist after %s and before %s", chain.group, chain.routing, chain.composite)
	}

	// codexDirect 链是一条 Use 调用，直接断言顺序。
	codexDirect := regexp.MustCompile(regexp.QuoteMeta(`codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), routeAPIKeyGroup, groupModelAllowlist, compositeTarget, requireGroupAnthropic)`))
	require.Regexp(t, codexDirect, source, "codexDirect chain must mount the allowlist after dynamic group selection and before compositeTarget")
