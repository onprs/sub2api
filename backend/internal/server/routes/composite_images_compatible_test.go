...
type compatibleImagesBillingEligibility struct{}

func (compatibleImagesBillingEligibility) ResolveUsableSubscriptionForRequest(context.Context, int64, int64, *int64, string) (*service.UserSubscription, error) {
	return nil, nil
}
...
func TestCompositeCompatibleImagesEndToEnd(t *testing.T) {
	...
	for _, scenario := range []string{"generation", "json_edit", "multipart_alias", "disabled", "native_only", "restricted"} {
		t.Run(scenario, func(t *testing.T) {
			...
			imagesHandler := handler.NewOpenAIGatewayHandler(gateway, nil, service.NewConcurrencyService(nil), billingCache, compatibleImagesBillingEligibility{}, service.NewAPIKeyService(nil, nil, nil, nil, nil, nil, cfg), nil, nil, nil, nil, cfg)
			publicModel := model
