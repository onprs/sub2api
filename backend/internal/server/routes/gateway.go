...
func RegisterGatewayRoutes(
	...
	// 根路径别名共用中间件链；动态分组选择必须先于所选分组的模型白名单校验。
	rootRoute := func(method, path string, limit gin.HandlerFunc, handler gin.HandlerFunc) {
		r.Handle(method, path, limit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), routeAPIKeyGroup, groupModelAllowlist, compositeTarget, requireGroupAnthropic, handler)
	}
	rootRoute(http.MethodPost, "/responses", bodyLimit, responsesHandler)
	rootRoute(http.MethodPost, "/responses/*subpath", bodyLimit, guardResponsesSubpath(func(c *gin.Context) {
		if isStandardProtocolGatewayPlatform(c) {
			writeOpenCodeGoResponsesSubpathUnsupported(c)
			return
		}
	...
	codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), routeAPIKeyGroup, groupModelAllowlist, compositeTarget, requireGroupAnthropic)
	{
	...
	providerV1.Use(groupModelAllowlist)
