...
func TestInvalidateCachePublishesToOtherInstances(t *testing.T) {
	...
	publisher := NewChannelServiceWithCachePubSub(&mockChannelRepository{}, nil, nil, nil, nil, cachePubSub)
	updated := false
	...
	subscriber := NewChannelServiceWithCachePubSub(subscriberRepo, nil, nil, nil, nil, cachePubSub)
