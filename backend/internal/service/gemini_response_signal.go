...
//nolint:unused // 仅保存有限前缀，供无 data 事件时的安全回放与判定。
type geminiSSEBodyCapture struct {
	buf       bytes.Buffer
	limit     int
	truncated bool
}

func (c *geminiSSEBodyCapture) Write(p []byte) (int, error) {
	if c == nil {
		return len(p), nil
	}
	if c.truncated {
...
func (c *geminiSSEBodyCapture) Bytes() []byte {
	if c == nil {
		return nil
	}
	return c.buf.Bytes()
...
func (c *geminiSSEBodyCapture) Truncated() bool {
	return c != nil && c.truncated
}

func geminiSSEFallbackFromRaw(raw []byte, truncated bool) *geminiSSEFallbackBody {
