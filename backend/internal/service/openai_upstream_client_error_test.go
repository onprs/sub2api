...
func TestFailoverOpenAIUpstreamHTTPError_ModelNotFoundIsNextAccountEligible(t *testing.T) {
	...
	failoverErr := svc.failoverOpenAIStructuredUpstreamError(
		context.Background(), c, account, upstream, "model not found", "missing-model",
	)
...
func TestHandleErrorResponse_MatchesCompatSiblingForDeterministic400(t *testing.T) {
	...
	compatResp := newOpenAIUpstreamErrorResponse(http.StatusBadRequest, openAIInvalidFunctionParametersBody)
	defer func() { _ = compatResp.Body.Close() }()
	compatUpstream, _, collectErr := svc.collectOpenAIStructuredUpstreamError(
		compatResp, protocolconv.ProtocolOpenAIResponses, false,
	)
