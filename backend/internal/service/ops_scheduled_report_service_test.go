...
func TestOpsScheduledReportLegacyTemplateReceivesSummaryHTML(t *testing.T) {
	...
	htmlBody := readNotificationEmailHTMLPart(t, smtpServer.lastMessage())
	require.Contains(t, htmlBody, `<section data-template="legacy">`)
	require.Contains(t, htmlBody, `<h2>日报</h2>`)
}

func readNotificationEmailHTMLPart(t *testing.T, rawMessage string) string {
	t.Helper()
	...
	for {
		...
		case "quoted-printable":
			content = quotedprintable.NewReader(part)
		}
	...
	require.Fail(t, "邮件中缺少 HTML MIME part")
	return ""
}
