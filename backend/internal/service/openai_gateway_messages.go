...
func (s *OpenAIGatewayService) ForwardAsAnthropic(
	...
	// 4. Apply OAuth-like Codex transport policy to the standard converted body.
	...
	if resp.StatusCode >= 400 {
		...
		// Grok account-switched history may still fail decryption after the
		// Responses-level retry. Strip client-side thinking signatures once so a
		// failover account can rebuild the request without encrypted history.
		if account.Platform == PlatformGrok &&
			isGrokInvalidEncryptedContentResponse(upstream.StatusCode, upstream.Body) &&
...
func (s *OpenAIGatewayService) handleAnthropicBufferedStreamingResponse(
	...
	terminal, err := s.collectOpenAICompatBufferedTerminal(resp, c, "openai messages buffered", startTime)
	if err != nil {
	...
	for {
		...
		case event, ok := <-events:
			...
			if event.err != nil {
				...
				return result, &openAICompatBufferedReadError{cause: event.err}
			}

			payloadBytes := []byte(openAICompatPayloadWithEventType(string(event.record.Data), event.record.Event))
			payloadBytes = restoreCodexToolNamesFromContext(c, payloadBytes)
			...
			s.parseSSEUsageBytesWithType(payloadBytes, parsed.Type, &result.Usage)
			acc.ProcessEvent(&parsed)
			response := openAICompatTerminalResponse(&parsed, payloadBytes)
			if !isOpenAICompatResponsesTerminalEvent(parsed.Type) || response == nil {
				continue
			}
			if parsed.Usage != nil {
			...
			result.Upstream = protocoltransport.Response{
				...
				RequestID: stream.RequestID, ResponseID: response.ID, Duration: time.Since(startTime),
			}
