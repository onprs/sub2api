...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	if input.APIKey != nil && input.APIKey.IsInternalChannelMonitor() {
		return nil
	}
	...
	cacheWriteInferred := inferMissingGPT56CacheWrite(result, apiKey.Group)
	...
	// 长上下文计费开关来自实际计费账号（shadow 账号回源到真实账号）。
	...
	var chargedBillingModel string
	cost, chargedBillingModel, err = s.calculateOpenAIRecordUsageCost(ctx, result, apiKey, billingModels, multiplier, imageMultiplier, videoMultiplier, baseMultiplier, tokens, serviceTier, longContextBillingEnabled)
	if err != nil {
	...
	if responseModel := responseModelBillingDeclaration(
		...
		if identified, responseChannelPriced := s.hasIdentifiedOpenAIResponsePricing(ctx, responseModel, apiKey); identified {
			...
			if responseErr == nil && responseModelBillingAdoptable(cost, responseCost, baselineChannelPriced, responseChannelPriced) {
				...
				chargedBillingModel = responseChargedModel
				cost = responseCost
			}
		}
	}
	...
	if isSubscriptionBilling && chargedBillingModel != "" {
...
func inferMissingGPT56CacheWrite(result *OpenAIForwardResult, group *Group) bool {
	if result == nil || group == nil || !group.ShouldInferGPT56CacheWrite() {
		return false
	}
	...
	if uncachedInput < group.GPT56CacheWriteInferenceMinTokens() {
		return false
	}
...
func (s *OpenAIGatewayService) calculateOpenAIRecordUsageCost(
	...
	if result != nil && result.AudioUsage != nil {
		...
		cost := s.billingService.CalculateAudioCost(result.AudioUsage.Mode, result.AudioUsage.DurationOrUnits, cfg, webSearchMultiplier)
		return cost, billingModel, nil
	}
	...
	if tokenCost == nil {
		...
		// 纯工具路径没有 token 模型时，允许仅按搜索调用计费。
		if searchCost != nil {
			return searchCost, billingModel, nil
		}
		if lastErr == nil {
