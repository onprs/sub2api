...
func TestTryModelFilePricing_Success(t *testing.T) {
	...
	tokens := UsageTokens{InputTokens: 100, OutputTokens: 50}
	result := tryModelFilePricingForPlatform(bs, "", "claude-sonnet-4", tokens, "", time.Time{})
	require.NotNil(t, result)
	// 100*0.001 + 50*0.002 = 0.1 + 0.1 = 0.2
	require.InDelta(t, 0.2, *result, 1e-12)
}
...
func TestTryModelFilePricing_Fable51MaxEffortUsesTripleQuota(t *testing.T) {
	...
	tokens := UsageTokens{InputTokens: 100}
	standard := tryModelFilePricingForPlatform(bs, "", "claude-fable-5-1", tokens, "", time.Time{}, "xhigh")
...
func TestTryModelFilePricing_AppliesLongContextPricing(t *testing.T) {
	...
	result := tryModelFilePricingForPlatform(bs, "", "gpt-5.6-sol", tokens, "", time.Time{})
...
func TestTryModelFilePricing_AppliesServiceTierPricing(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := tryModelFilePricingForPlatform(bs, "", "gpt-5.6-sol", tokens, tt.serviceTier, time.Time{})
			require.NotNil(t, result)
...
func TestTryModelFilePricing_CombinesPriorityAndLongContextPricing(t *testing.T) {
	...
	result := tryModelFilePricingForPlatform(bs, "", "gpt-5.6-sol", tokens, "priority", time.Time{})
...
func TestTryModelFilePricing_PricingNotFound(t *testing.T) {
	...
	bs := newTestBillingServiceWithPrices(map[string]*ModelPricing{})
	tokens := UsageTokens{InputTokens: 100, OutputTokens: 50}
	result := tryModelFilePricingForPlatform(bs, "", "nonexistent-model", tokens, "", time.Time{})
	require.Nil(t, result)
}
			...
			t.Run(model.name+"/"+usage.name, func(t *testing.T) {
				...
				for _, slot := range []struct {
					...
					t.Run(slot.name, func(t *testing.T) {
						cost := tryModelFilePricingForPlatform(bs, "", model.name, tokens, "", slot.at)
