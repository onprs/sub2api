...
func (s *OpenAIQuotaService) resetCredit(ctx context.Context, accountID int64, creditID, redeemRequestID string, targeted bool) (*OpenAIQuotaResetResult, error) {
	...
	// targeted 自动消费在调用方统一执行账号恢复和额度刷新；手动调用保留
	// 原有即时清限流与快照更新行为，避免同一次自动流程重复请求 usage。
	if !targeted {
		if s.accountRepo != nil {
		...
		// Agent Identity 的重置请求已经完成独立认证与一次失效恢复；避免在同一操作中
		// 再发起 usage/detail 请求，减少额外断言并把快照刷新交给后续用量查询。
		if !agentIdentity {
