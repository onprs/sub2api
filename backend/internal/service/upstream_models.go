...
func (s *AccountTestService) fetchUpstreamModelList(ctx context.Context, account *Account) ([]string, []byte, error) {
	...
	if account.IsOpenRouterAPIKey() {
		...
		if len(models) == 0 {
			return nil, nil, newUpstreamModelSyncUpstreamError("OpenRouter model catalog is unavailable", nil)
		}
		return models, nil, nil
	}
	if account.IsCommandCodeAPIKey() {
		...
		if len(models) == 0 {
			return nil, nil, newUpstreamModelSyncUpstreamError("Command Code GOAT model catalog is unavailable", nil)
		}
		return models, nil, nil
	}
...
func (s *AccountTestService) buildOpenAIUpstreamModelsRequest(ctx context.Context, account *Account) (*http.Request, error) {
	...
	return buildOpenAIAPIKeyModelsRequest(ctx, account, s.validateOpenAIAPIKeyBaseURL)
}
...
// OAuth 订阅不暴露公共 Platform API 的 /v1/models，不能按 API Key 账户处理。
...
func (s *AccountTestService) FetchAntigravityAccountCatalog(ctx context.Context, account *Account) ([]antigravity.CatalogModel, error) {
	...
	modelsResp, raw, err := client.FetchAvailableModels(
		ctx,
		accessToken,
		projectID,
