...
func TestOpenAIGatewayService_OAuthLegacy_GroupForceOpenAIFastInjectsMissingTier(t *testing.T) {
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		...
		Body: io.NopCloser(strings.NewReader(
			`data: {"type":"response.completed","response":{"id":"resp_group_fast_legacy","object":"response","model":"gpt-5.6-sol","status":"completed","output":[],"usage":{"input_tokens":1,"output_tokens":1,"total_tokens":2}}}` + "\n\n" +
				"data: [DONE]\n\n",
		)),
	}}
...
func TestOpenAIGatewayService_OAuthLegacy_GroupForceStillHonorsGlobalFilter(t *testing.T) {
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		...
		Body: io.NopCloser(strings.NewReader(
			`data: {"type":"response.completed","response":{"id":"resp_group_fast_filter","object":"response","model":"gpt-5.6-sol","status":"completed","output":[],"usage":{"input_tokens":1,"output_tokens":1,"total_tokens":2}}}` + "\n\n" +
				"data: [DONE]\n\n",
		)),
	}}
...
func TestOpenAIGatewayService_CodexFingerprintHTTPTransformedHeaderBodyParityAndDefaultCacheKey(t *testing.T) {
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		...
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid"}},
		Body: io.NopCloser(strings.NewReader(
			`data: {"type":"response.completed","response":{"id":"resp_fingerprint","object":"response","model":"gpt-5.2","status":"completed","output":[{"type":"message","id":"msg_fingerprint","role":"assistant","status":"completed","content":[{"type":"output_text","text":"ok"}]}],"usage":{"input_tokens":1,"output_tokens":1,"total_tokens":2}}}` + "\n\ndata: [DONE]\n\n",
		)),
	}}
	svc := &OpenAIGatewayService{
		cfg:           &config.Config{},
