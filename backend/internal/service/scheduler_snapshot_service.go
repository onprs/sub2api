...
func (s *SchedulerSnapshotService) ListSchedulableAccounts(ctx context.Context, groupID *int64, platform string, hasForcePlatform bool) ([]Account, bool, error) {
	...
	accounts, err := s.loadAndCacheAccounts(ctx, bucket, useMixed, writeToken, canPublish)
	return accounts, useMixed, err
}

func (s *SchedulerSnapshotService) loadAndCacheAccounts(ctx context.Context, bucket SchedulerBucket, useMixed bool, writeToken SchedulerBucketWriteToken, canPublish bool) ([]Account, error) {
...
func (s *SchedulerSnapshotService) loadAccountsAuthoritatively(ctx context.Context, bucket SchedulerBucket, useMixed bool) ([]Account, bool, error) {
	...
	resultCh := s.authoritativeRefresh.DoChan(bucket.String(), func() (any, error) {
		...
		defer cancel()
		// 权威重读后回写 cache（HEAD 语义）：避免旧快照持续失效；
		// token 捕获失败时退化为不发布（与 loadAndCacheAccounts 一致）。
		writeToken := SchedulerBucketWriteToken{}
		canPublish := false
		if s.cache != nil {
			if token, tokenErr := s.cache.CaptureBucketWriteToken(refreshCtx, bucket); tokenErr == nil {
				writeToken = token
				canPublish = true
			}
		}
		accounts, err := s.loadAndCacheAccounts(refreshCtx, bucket, useMixed, writeToken, canPublish)
...
func schedulerSnapshotPlatforms() [9]string {
	return [9]string{PlatformAnthropic, PlatformGemini, PlatformOpenAI, PlatformAntigravity, PlatformGrok, PlatformOpenCodeGo, PlatformClinePass, PlatformOpenRouter, PlatformCommandCode}
}
...
// defaultBuckets 返回 group0 的 9 平台桶 + 活跃分组桶；simple 模式或 groupRepo 缺失时仅返回 group0。
func (s *SchedulerSnapshotService) defaultBuckets(ctx context.Context) ([]SchedulerBucket, error) {
	buckets := make([]SchedulerBucket, 0)
	platforms := []string{PlatformAnthropic, PlatformGemini, PlatformOpenAI, PlatformAntigravity, PlatformGrok, PlatformOpenCodeGo, PlatformClinePass, PlatformOpenRouter, PlatformCommandCode}
