...
func (s *OpenAIGatewayService) SelectAccountForModelWithExclusions(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}) (*Account, error) {
	return selectWithSchedulerSnapshotRetry(ctx, isNoAvailableAccountSelectionError, func(attemptCtx context.Context) (*Account, error) {
		return s.selectAccountForModelWithExclusions(s.withOpenAIQuotaAutoPauseContext(attemptCtx), groupID, PlatformOpenAI, sessionHash, requestedModel, excludedIDs, false, 0, "", false)
	})
}
...
// Raw Chat fallback 使用普通账号映射和上游归一化，不使用 compact 映射。
func resolveOpenAIAccountUpstreamModelForRequest(account *Account, requestedModel string, requireCompact bool) string {
	...
	// Passthrough 账号只替换鉴权，通常保持渠道映射后的模型；旧式 compact
	// 是唯一会直接在该模型上应用 compact_model_mapping 的例外。
...
func (s *OpenAIGatewayService) selectAccountForModelWithExclusions(ctx context.Context, groupID *int64, platform string, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, stickyAccountID int64, requiredCapability OpenAIEndpointCapability, preferLowUpstreamRateOptions ...bool) (*Account, error) {
	preferLowUpstreamRate := len(preferLowUpstreamRateOptions) > 0 && preferLowUpstreamRateOptions[0]
	platform = NormalizeOpenAICompatiblePlatform(platform)
...
func (s *OpenAIGatewayService) SelectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}) (*AccountSelectionResult, error) {
	return selectWithSchedulerSnapshotRetry(ctx, isNoAvailableAccountSelectionError, func(attemptCtx context.Context) (*AccountSelectionResult, error) {
		...
		attemptCtx = s.withOpenAIGroupPrivacyRequirement(attemptCtx, groupID)
		// legacy 公共入口同样装门，保证不经 selectAccountWithScheduler 的调用方
		// 也无法绕过利润准入；每次权威重试都在本次 attempt context 上重装。
		attemptCtx = s.withOpenAIProfitControlGate(attemptCtx, groupID)
...
func (s *OpenAIGatewayService) recheckSelectedOpenAIAccountFromDBBeforeProfit(ctx context.Context, account *Account, groupID *int64, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) *Account {
	...
	groupMatched := s.openAIAccountMatchesSchedulingGroup(latest, groupID)
	if !groupMatched {
		// 权威列表查询已经按分组过滤；部分轻量仓库实现不会在 GetByID
		// 返回分组字段，因此仅对本次权威列表明确返回且缺少分组元数据的账号信任该结果。
		_, authoritative := schedulerAuthoritativeAccountFromContext(ctx, latest.ID)
		if !authoritative || len(latest.GroupIDs) > 0 || len(latest.AccountGroups) > 0 {
			return nil
		}
	}
...
func (s *OpenAIGatewayService) hydrateSelectedAccount(ctx context.Context, account *Account) (*Account, error) {
	...
	if !hydrated.IsSchedulable() || !hydrated.IsOpenAICompatible() {
		return nil, ErrNoAvailableAccounts
	}
	return hydrated, nil
}
