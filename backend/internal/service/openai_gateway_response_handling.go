...
func (s *OpenAIGatewayService) handleStreamingResponseWithOutputAndReasoning(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, startTime time.Time, originalModel, mappedModel string, output openAIProtocolOutput, reasoningEffort string) (*openaiStreamingResult, error) {
	...
	return s.handleStreamingResponseWithReasoning(ctx, resp, c, account, startTime, originalModel, mappedModel, reasoningEffort)
}

func (s *OpenAIGatewayService) handleStreamingResponseWithReasoning(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, startTime time.Time, originalModel, mappedModel, reasoningEffort string) (*openaiStreamingResult, error) {
	return s.handleStreamingResponseWithReasoningImpl(ctx, resp, c, account, startTime, originalModel, mappedModel, reasoningEffort)
}

func (s *OpenAIGatewayService) handleStreamingResponseWithReasoningImpl(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, startTime time.Time, originalModel, mappedModel, reasoningEffort string) (*openaiStreamingResult, error) {
	output := openAIProtocolOutput(nil)
	...
	// Attempt-local headers remain private until a complete semantic event wins.
	...
	sendErrorEvent := func(reason string) {
		if output != nil || errorEventSent || clientDisconnected || failureDelivered {
			return
		}
		errorEventSent = true
	...
	processSSELine := func(line string, queueDrained bool) {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			if output == nil && needModelReplace && mappedModel != "" && strings.Contains(line, mappedModel) {
			...
			// 写入客户端（客户端断开后继续 drain 上游）。Structured
			// output consumes normalized Responses JSON without SSE framing.
			if output != nil {
				if !failureDelivered && !suppressCurrentEvent && strings.TrimSpace(data) != "[DONE]" {
		...
		if output == nil && !clientDisconnected && !failureDelivered && !suppressCurrentEvent {
			if _, err := writePendingString(line); err != nil {
				handlePendingWriteError(err)
			} else if _, err := writePendingString("\n"); err != nil {
				handlePendingWriteError(err)
	...
	for {
		...
		case <-keepaliveCh:
			if output != nil || clientDisconnected || failureDelivered || eventInProgress {
				continue
			}
...
func (s *OpenAIGatewayService) handleNonStreamingResponse(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, originalModel, mappedModel string) (*openaiNonStreamingResult, error) {
	return s.handleNonStreamingResponseWithOutput(ctx, resp, c, account, originalModel, mappedModel, nil)
}

func (s *OpenAIGatewayService) handleNonStreamingResponseWithOutput(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, originalModel, mappedModel string, output openAIProtocolOutput) (*openaiNonStreamingResult, error) {
	...
	logOpenAISuccessMissingUsage(ctx, c, account, resp, result.usage, "json", false)

	if output != nil {
		structured.Body, err = restoreGrokResponsesClientToolPayload(c, structured.Body)
		if err != nil {
			return nil, fmt.Errorf("restore Grok Responses client tool response: %w", err)
		}
		structured.Body, err = restoreOpenAIResponsesClientToolPayload(c, structured.Body)
		if err != nil {
			return nil, fmt.Errorf("restore OpenAI Responses client tool response: %w", err)
		}
		structured.Body, err = restoreOpenAIResponsesNamespacePayload(c, structured.Body)
		if err != nil {
			return nil, fmt.Errorf("restore OpenAI namespace response: %w", err)
		}
		structured.Body = restoreCodexToolNamesFromContext(c, structured.Body)
	...
	restoredBody, restoreErr := restoreGrokResponsesClientToolPayload(c, downstreamBody)
	if restoreErr != nil {
	...
	downstreamBody = restoreCodexToolNamesFromContext(c, restoredBody)
	responseheaders.WriteFilteredHeaders(c.Writer.Header(), resp.Header, s.responseHeaderFilter)
...
func (s *OpenAIGatewayService) handleSSEToJSONWithOutput(resp *http.Response, c *gin.Context, account *Account, body []byte, originalModel, mappedModel string, output openAIProtocolOutput) (*openaiNonStreamingResult, error) {
	...
	if output != nil {
		if !ok {
			return nil, errors.New("OpenAI Responses SSE ended without a complete terminal response")
		}
		structured := protocoltransport.Response{
