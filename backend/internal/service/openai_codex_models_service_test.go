...
func TestFetchCodexModelsManifestSetupTokenUsesBearerCredentials(t *testing.T) {
	...
	account.Credentials["access_token"] = "setup-access-token"

	manifest, err := (&OpenAIGatewayService{}).FetchCodexModelsManifest(context.Background(), account, "0.137.0", "")
	require.NoError(t, err)
	require.Equal(t, `{"models":[{"slug":"gpt-5.6-sol"}]}`, string(manifest.Body))
	require.Equal(t, "Bearer setup-access-token", gotAuth)
...
func TestSelectCodexModelsManifestAccountSelectsSetupToken(t *testing.T) {
	groupID := int64(99)
	repo := groupAwareStubOpenAIAccountRepo{stubOpenAIAccountRepo{accounts: []Account{
		{
			ID:          3,
	...
	require.Equal(t, AccountTypeSetupToken, selected.Type)
}
...
func TestFetchCodexModelsManifestSetupToken401DisablesAccount(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(http.StatusUnauthorized)
		_, _ = w.Write([]byte(`{"detail":{"message":"invalid setup token"}}`))
	}))
	defer server.Close()
	...
	require.Error(t, err)
	require.True(t, IsRetryableCodexModelsManifestError(err), "manifest 401 should allow account failover")
	require.Equal(t, 1, repo.setErrorCalls, "setup-token 401 should permanently disable the non-refreshable account")
