...
var (
	...
	ErrChannelMonitorTemplateInvalidProvider = infraerrors.BadRequest(
		"CHANNEL_MONITOR_TEMPLATE_INVALID_PROVIDER", "template provider must be one of openai/anthropic/gemini/opencode/clinepass/openrouter/commandcode/antigravity_claude/antigravity_gemini/grok",
	)
	ErrChannelMonitorTemplateInvalidAPIMode = infraerrors.BadRequest(
		"CHANNEL_MONITOR_TEMPLATE_INVALID_API_MODE", "template api_mode must be chat_completions, messages, or responses; responses is supported for openai/opencode and messages is supported for opencode/commandcode",
	)
