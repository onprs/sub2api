...
func TestCalculateTokenCostForRequest_ReasoningPreservesPlatformPricing(t *testing.T) {
	bs := NewBillingService(&config.Config{}, nil)
	tokens := UsageTokens{InputTokens: 272001, OutputTokens: 1}
	want, err := bs.CalculateCostForPlatform(PlatformCommandCode, "gpt-5.6-luna", tokens, 1)
	require.NoError(t, err)
	for _, effort := range []string{"medium", "max"} {
		got, err := bs.CalculateTokenCostForRequest(TokenCostRequest{
			Model: "gpt-5.6-luna", PricingPlatform: PlatformCommandCode,
			Tokens: tokens, RateMultiplier: 1, ReasoningEffort: effort,
		})
		require.NoError(t, err)
		require.InDelta(t, want.TotalCost, got.TotalCost, 1e-12, effort)
	}
}
