...
func TestBufferRawChatCompletionsUsesIdentityPipelineAndRenderer(t *testing.T) {
	...
	result, err := svc.bufferRawChatCompletions(
		rec.Context, resp, rawChatCompletionsTestAccount(), pipeline, "client-model", "billing-model", "upstream-model", nil, nil, time.Now(),
	)
...
func TestStreamRawChatCompletionsRejectsMalformedJSONBeforeCommit(t *testing.T) {
	...
	require.Equal(t, OpenAIUpstreamStreamReadErrorCode,
...
func TestBufferRawChatCompletionsPreservesEmptySuccessResponse(t *testing.T) {
	...
	result, err := (&OpenAIGatewayService{}).bufferRawChatCompletions(
		c, resp, rawChatCompletionsTestAccount(), pipeline, "client-model", "billing-model", "upstream-model", nil, nil, time.Now(),
	)
...
func TestBufferRawChatCompletions_RejectsOversizedResponse(t *testing.T) {
	...
	result, err := svc.bufferRawChatCompletions(c, resp, rawChatCompletionsTestAccount(), pipeline, "gpt-5.4", "gpt-5.4", "gpt-5.4", nil, nil, time.Now())
