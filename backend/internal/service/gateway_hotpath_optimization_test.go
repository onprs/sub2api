...
func TestGetAvailableModels_FiltersOnlyModelsCooledDownOnEverySupportingAccount(t *testing.T) {
	groupID := int64(58)
	activeReset := time.Now().Add(30 * time.Minute).UTC().Format(time.RFC3339)
	expiredReset := time.Now().Add(-time.Minute).UTC().Format(time.RFC3339)
	modelLimit := func(resetAt string) map[string]any {
		return map[string]any{"rate_limit_reset_at": resetAt, "reason": "upstream_404_model_not_found"}
	}
	repo := &modelsListAccountRepoStub{
		byGroup: map[int64][]Account{
			groupID: {
				{
					...
					Credentials: map[string]any{"model_mapping": map[string]any{
						"gpt-5.2": "gpt-5.2", "gpt-5.4": "gpt-5.4", "stable-alias": "stable-upstream",
					}},
					Extra: map[string]any{"model_rate_limits": map[string]any{
						"gpt-5.2": modelLimit(activeReset), "gpt-5.4": modelLimit(activeReset), "stable-upstream": modelLimit(expiredReset),
					}},
				},
				{
					ID: 2, Platform: PlatformOpenAI, Type: AccountTypeOAuth,
					Credentials: map[string]any{"model_mapping": map[string]any{
						"gpt-5.2": "gpt-5.2", "gpt-5.4": "gpt-5.4",
					}},
					Extra: map[string]any{"model_rate_limits": map[string]any{
						"gpt-5.2": modelLimit(activeReset),
					}},
				},
			},
		},
	...
	models := svc.GetAvailableModels(context.Background(), &groupID, PlatformOpenAI)
	require.NotContains(t, models, "gpt-5.2", "all supporting accounts are in active cooldown")
	require.Contains(t, models, "gpt-5.4", "one supporting account remains available")
	require.Contains(t, models, "stable-alias", "expired mapped-model cooldown must not hide alias")
}

func TestGetAvailableModels_PreservesExplicitEmptyResultAcrossCache(t *testing.T) {
	groupID := int64(59)
	activeReset := time.Now().Add(30 * time.Minute).UTC().Format(time.RFC3339)
		...
		groupID: {
			{
				...
				Extra: map[string]any{"model_rate_limits": map[string]any{
					"gpt-5.2": map[string]any{"rate_limit_reset_at": activeReset},
				}},
			},
		},
	}}
	...
	second := svc.GetAvailableModels(context.Background(), &groupID, PlatformOpenAI)
	require.NotNil(t, second, "cache clone must preserve explicit empty result")
