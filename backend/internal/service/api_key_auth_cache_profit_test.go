...
func TestAPIKeyAuthSnapshotProfitControlRoundtrip(t *testing.T) {
	...
	require.Equal(t, 21, snapshot.Version, "v21 起动态候选分组同时携带利润控制与 search/audio/video_model_prices 计费字段")
	...
	require.Len(t, materialized.RoutingGroups, 1)
	dynamicGroup := materialized.RoutingGroups[0].Group
	require.NotNil(t, dynamicGroup)
