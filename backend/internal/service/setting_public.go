...
// AvailableChannelsRuntime is the lightweight view of user-facing channel
// aggregation feature switches consumed by the user-facing handler.
...
// GetModelPricingRuntime reads the model-pricing feature switch independently
// from available-channels so the two user-facing entries can be enabled separately.
func (s *SettingService) GetModelPricingRuntime(ctx context.Context) AvailableChannelsRuntime {
