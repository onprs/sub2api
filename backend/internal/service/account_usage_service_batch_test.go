...
func TestAccountUsageService_GetUsageBatch_BestEffortByAccount(t *testing.T) {
	...
	usageByAccount, errorsByAccount, err := svc.GetUsageBatch(context.Background(), []int64{7001, 7002, 7003, 7004, 7002}, false)
	if err != nil {
	...
	if openAIAPIKeyUsage == nil || openAIAPIKeyUsage.UpstreamBalance == nil ||
		openAIAPIKeyUsage.UpstreamBalance.Status != openAIUpstreamBalanceStatusUnsupported {
		t.Fatalf("expected OpenAI API key live balance to degrade as unsupported, got %#v", openAIAPIKeyUsage)
	}
	if errorsByAccount[7004] != "" {
		t.Fatalf("expected OpenAI API key live balance degradation without batch error, got %q", errorsByAccount[7004])
	}
}
