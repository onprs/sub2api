...
type imageStorageRoundTripFunc func(*http.Request) (*http.Response, error)

func (f imageStorageRoundTripFunc) RoundTrip(req *http.Request) (*http.Response, error) {
	return f(req)
}
...
func TestImageResultUploaderRewritesImageDataURLWithoutHTTP(t *testing.T) {
	httpCalls := 0
	client := &http.Client{Transport: imageStorageRoundTripFunc(func(*http.Request) (*http.Response, error) {
		httpCalls++
