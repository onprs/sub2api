...
func (s *OpenAIGatewayService) forwardGrokResponses(
	...
	if resp.StatusCode >= 400 {
		...
		// 配额/限流按团队和模型设置冷却；请求级容量压力不能隐藏同团队其他账户。
		if shouldMarkGrokTeamModelRateLimit(upstream.StatusCode, upstream.Body) {
...
func addOpenAIUsage(dst *OpenAIUsage, usage OpenAIUsage) {
	...
	dst.CacheCreation5mTokens += usage.CacheCreation5mTokens
	dst.CacheCreation1hTokens += usage.CacheCreation1hTokens
...
func (s *OpenAIGatewayService) updateGrokUsageSnapshotWithRateLimit(ctx context.Context, account *Account, snapshot *xai.QuotaSnapshot, installRateLimit bool) {
	...
	// 错误响应由 handleGrokAccountUpstreamError 统一处理。池模式账号只保留快照，不写入账号级限流状态。
