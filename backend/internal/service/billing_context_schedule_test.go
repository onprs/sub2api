...
func tierPricePointer(tier ContextPricingTier, kind tokenKind) *float64 {
	switch kind {
	case kindInput:
		return tier.Input
	case kindOutput:
		return tier.Output
	case kindCacheRead:
		return tier.CacheRead
	case kindCacheWrite:
		return tier.CacheWrite
	default:
		return nil
	}
}
...
func contextPricingComponentAvailable(s *ContextPricingSchedule, kind tokenKind, contextTokens int) bool {
	if s == nil || len(s.Tiers) == 0 {
		return false
	}
	if s.Basis != ContextPricingBasisMarginal {
		return tierPricePointer(tierAt(s.Tiers, contextTokens), kind) != nil
	}
	for _, tier := range s.Tiers {
		if contextTokens <= tier.MinTokens {
			break
		}
		if tierPricePointer(tier, kind) == nil {
			return false
		}
	}
	return true
...
func TestResolveContextPricingSchedule_ParityWithBilling(t *testing.T) {
	...
	for _, sc := range scheduleScenarios() {
		...
		t.Run(sc.name, func(t *testing.T) {
			...
			pricingInput := PricingInput{Model: sc.model, Platform: sc.platform, Group: sc.group}
			if sc.group != nil {
				gid := sc.group.ID
			...
			strictComponentPricing := legacy == nil && sc.group != nil
			cost := func(tokens UsageTokens) (float64, error) {
				bd, err := bs.CalculateTokenCostForRequest(TokenCostRequest{
					Ctx: ctx, Model: sc.model, PricingPlatform: sc.platform, Group: sc.group, Tokens: tokens, RateMultiplier: 1,
					Resolver: resolver, Resolved: resolved, LegacyLongContext: legacy,
				})
				if err != nil {
					return 0, err
				}
				return bd.ActualCost, nil
			}
			...
			for _, c := range probes {
				...
				assertComponent := func(kind tokenKind, tokens UsageTokens, label string) {
					got, costErr := cost(tokens)
					if !contextPricingComponentAvailable(sched, kind, c) && strictComponentPricing {
						require.ErrorIsf(t, costErr, ErrModelPricingUnavailable, "%s@%d", label, c)
						return
					}
					require.NoErrorf(t, costErr, "%s@%d", label, c)
					assertCostClose(t, expectedCostFromSchedule(sched, kind, c), got, "%s@%d", label, c)
				}
				assertComponent(kindInput, UsageTokens{InputTokens: c}, "input")
				assertComponent(kindCacheRead, UsageTokens{CacheReadTokens: c}, "cache_read")
				assertComponent(kindCacheWrite, UsageTokens{CacheCreationTokens: c}, "cache_write")

				withOutput, outputErr := cost(UsageTokens{InputTokens: c, OutputTokens: outputProbe})
				withoutOutput, inputErr := cost(UsageTokens{InputTokens: c})
				componentsAvailable := contextPricingComponentAvailable(sched, kindInput, c) &&
					contextPricingComponentAvailable(sched, kindOutput, c)
				if !componentsAvailable && strictComponentPricing {
					require.ErrorIsf(t, outputErr, ErrModelPricingUnavailable, "output@%d", c)
					continue
				}
				require.NoErrorf(t, inputErr, "input baseline@%d", c)
				require.NoErrorf(t, outputErr, "output@%d", c)
				wantOutput := float64(outputProbe) * tierPrice(tierAt(sched.Tiers, c), kindOutput)
				assertCostClose(t, wantOutput, withOutput-withoutOutput, "output@%d", c)
			}
		})
	}
}
