...
// GrokMediaVideoRequestOwner 是视频 request_id 的本地访问主体。
...
// GrokMediaVideoBindingError 表示上游已接受生成请求，但本地归属尚未可靠保存。
type GrokMediaVideoBindingError struct {
	Err error
}

func (e *GrokMediaVideoBindingError) Error() string {
	return "persist grok video request ownership: " + e.Err.Error()
}

func (e *GrokMediaVideoBindingError) Unwrap() error { return e.Err }
...
func (s *OpenAIGatewayService) grokMediaVideoRequestBindingTTL() time.Duration {
...
func (s *OpenAIGatewayService) BindGrokMediaVideoRequestAccount(
	...
	return s.cache.SetSessionAccountID(ctx, derefGroupID(groupID), cacheKey, accountID, s.grokMediaVideoRequestBindingTTL())
}
...
func (s *OpenAIGatewayService) ResolveGrokMediaVideoRequestAccount(
	...
	// 当前查询已拥有有效归属；续租失败不应把本次可证明的查询降级成 404。
	_ = s.cache.RefreshSessionTTL(ctx, derefGroupID(groupID), cacheKey, s.grokMediaVideoRequestBindingTTL())
...
// ForwardGrokMediaWithVideoOwner 在响应提交前保存视频请求归属。
func (s *OpenAIGatewayService) ForwardGrokMediaWithVideoOwner(
	ctx context.Context,
	c *gin.Context,
	account *Account,
	endpoint GrokMediaEndpoint,
	...
	return s.forwardGrokMedia(ctx, c, account, endpoint, requestID, body, contentType, owner)
}
...
func (s *OpenAIGatewayService) forwardGrokMedia(
	...
	if endpoint.IsVideoCreateRequest() && strings.TrimSpace(usage.ResponseID) != "" && owner != nil {
		if err := s.BindGrokMediaVideoRequestAccount(
			ctx,
			owner.GroupID,
			usage.ResponseID,
			owner.UserID,
			...
			return nil, &GrokMediaVideoBindingError{Err: err}
		}
	}
