...
func (a *Account) resolveModelMapping(rawMapping map[string]any) map[string]string {
	...
	result := stringMappingFromRaw(rawMapping)
	if len(result) > 0 {
		return result
	}
	if a.IsGeminiGoogleOne() {
...
// 会把未知模型原样透传，上游会返回不可重试的 400，导致请求卡死在该账号上、
// 无法 failover 到真正支持该模型的 API Key 账号（#3662）。未知/自定义别名
// 仍保持允许（兼容渠道级映射），见 isOpenAIOAuthServableModel。
