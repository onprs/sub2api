...
// 家族的模型（deepseek-*/glm-* 等）和已知不受 ChatGPT Codex 支持的模型。
// 转发阶段 normalizeOpenAIModelForUpstream 会把未知模型原样透传，上游会返回
// 不可重试的 400，导致请求卡死在该账号上、无法 failover 到真正支持该模型的
// API Key 账号（#3662）。未知/自定义别名仍保持允许（兼容渠道级映射），见
// isOpenAIOAuthServableModel。
