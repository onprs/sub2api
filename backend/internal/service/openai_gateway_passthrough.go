...
func (s *OpenAIGatewayService) forwardOpenAIPassthrough(
	...
	if resp.StatusCode >= 400 {
		...
		if shouldFailoverOpenAIPassthroughResponse(account, upstream.StatusCode, upstream.Body) {
			return nil, s.handleFailoverErrorResponsePassthrough(ctx, upstream, c, account, body, upstream.Body)
		}
		return nil, s.handleErrorResponsePassthrough(ctx, upstream, c, account, body, upstream.Body)
	}
...
func (s *OpenAIGatewayService) handleNonStreamingResponsePassthroughWithOutput(
	...
	if output != nil {
		...
		if isNativeResponsesProtocolOutput(output) {
			...
			if mapping, ok := openAIResponsesClientToolMapping(c); ok && json.Valid(structuredBody) {
				structuredBody, _, err = apicompat.RestoreResponsesClientToolPayload(structuredBody, mapping)
				if err != nil {
