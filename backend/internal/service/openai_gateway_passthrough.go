	...
	for {
		...
		if resp.StatusCode >= 400 {
			...
			upstream := protocoltransport.Response{
				...
				Body: probeBody, ActualProtocol: protocolconv.ProtocolOpenAIResponses,
				RequestID: resp.Header.Get("x-request-id"),
			}
			if err := upstream.Validate(); err != nil {
				return nil, fmt.Errorf("validate OpenAI passthrough error response: %w", err)
			}
			if shouldFailoverOpenAIPassthroughResponse(account, upstream.StatusCode, probeBody) {
				return nil, s.handleFailoverErrorResponsePassthrough(ctx, upstream, c, account, body, probeBody)
			}
			return nil, s.handleErrorResponsePassthrough(ctx, upstream, c, account, body, probeBody)
		}
		...
		bodyOwnedByHandler := output != nil && (reqStream || isEventStreamResponse(resp.Header))
		var handleErr error
		if reqStream {
			...
			result, err := s.handleNonStreamingResponsePassthroughWithOutput(ctx, resp, c, account, reqModel, upstreamPassthroughModel, output)
			handleErr = err
			if result != nil {
				usage = result.usage
				responseID = strings.TrimSpace(result.responseID)
		...
		if handleErr != nil {
			...
			if signal, ok := asOpenAICompactFallbackSignal(handleErr); ok {
				...
				compactUpstream := protocoltransport.Response{
					StatusCode: compactResp.StatusCode, Headers: compactResp.Header.Clone(),
					Body: compactBody, ActualProtocol: protocolconv.ProtocolOpenAIResponses,
					RequestID: compactResp.Header.Get("x-request-id"),
				}
				_ = compactResp.Body.Close()
				if shouldFailoverOpenAIPassthroughResponse(account, compactUpstream.StatusCode, compactBody) {
					return nil, s.handleFailoverErrorResponsePassthrough(ctx, compactUpstream, c, account, body, compactBody)
				}
				return nil, s.handleErrorResponsePassthrough(ctx, compactUpstream, c, account, body, compactBody)
			}
			if !bodyOwnedByHandler {
				_ = resp.Body.Close()
			}
		...
		// 只有原生 Responses 输出会下发 turn-state；成功写出后再记录铸造账号，
		// 避免跨协议输出或失败 attempt 污染溯源。
		if (output == nil || isNativeResponsesProtocolOutput(output)) && extractOpenAICodexTurnState(resp.Header) != "" {
...
func (s *OpenAIGatewayService) handleFailoverErrorResponsePassthrough(
	...
	shouldDisable := s.handleOpenAIAccountUpstreamError(ctx, account, upstream.StatusCode, upstream.Headers, body, canonicalModel)
	...
	return s.newOpenAIAccountFailoverError(
		...
		upstreamMsg,
		shouldDisable,
		!shouldDisable && account.IsPoolMode() && account.IsPoolModeRetryableStatus(upstream.StatusCode),
	)
}
...
func (s *OpenAIGatewayService) handleErrorResponsePassthrough(
	...
	if !cyberHit {
		reqModel, _, _ := extractOpenAIRequestMetaFromBody(requestBody)
		canonicalModel := canonicalOpenAIAccountSchedulingModel(account, reqModel)
		_ = s.handleOpenAIAccountUpstreamError(ctx, account, upstream.StatusCode, upstream.Headers, body, canonicalModel)
	}
	appendOpsUpstreamError(c, OpsUpstreamErrorEvent{
		ProxyID:              opsUpstreamProxyID(account),
		ProxyName:            opsUpstreamProxyName(account),
	...
	if isOpenAIContextWindowError(upstreamMsg, body) && upstreamMsg != "" {
		writeOpenAIPassthroughErrorEnvelope(c, upstream.StatusCode, upstream.Headers, upstreamMsg)
	} else if account != nil && account.Type == AccountTypeOAuth {
		// HEAD 语义：OAuth 透传保持原样代理，错误体直接回写客户端（仅净化
		// agent identity 敏感字段）。
		...
		// 官方语义：APIKey 上游错误可能携带敏感信息（密钥、内部字段），
		// 统一重建为净化信封；返回错误也不携带上游消息，避免泄漏。
		writeSanitizedOpenAIPassthroughError(c, upstream.StatusCode, upstream.Headers)
		return fmt.Errorf("upstream error: %d (client response sanitized)", upstream.StatusCode)
	}
...
func (s *OpenAIGatewayService) handleStreamingResponsePassthroughWithOutput(
	...
	for documentScanner.Scan() {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			if output == nil && needModelReplace && strings.Contains(data, `"model"`) {
...
func (s *OpenAIGatewayService) handleNonStreamingResponsePassthrough(
	...
	return s.handleNonStreamingResponsePassthroughWithOutput(ctx, resp, c, account, originalModel, mappedModel, nil)
}
...
func (s *OpenAIGatewayService) handleNonStreamingResponsePassthroughWithOutput(
	...
	if output != nil && isEventStreamResponse(resp.Header) {
		return s.handleStructuredPassthroughSSEToJSONWithOutput(ctx, resp, c, account, output)
	}
	...
	if isEventStreamResponse(resp.Header) {
		return s.handlePassthroughSSEToJSONWithOutput(resp, c, account, body, originalModel, mappedModel, output)
	}
	...
	if output != nil {
		...
		structuredBody := append([]byte(nil), body...)
		structuredBody, err = restoreOpenAIResponsesClientToolPayload(c, structuredBody)
		if err != nil {
			return nil, fmt.Errorf("restore OpenAI Responses client tools: %w", err)
		}
		structuredBody, err = restoreOpenAIResponsesNamespacePayload(c, structuredBody)
		if err != nil {
			return nil, fmt.Errorf("restore OpenAI passthrough namespace response: %w", err)
		}
		structuredBody = restoreCodexToolNamesFromContext(c, structuredBody)
		...
		structured := protocoltransport.Response{
			...
			Body: structuredBody, ActualProtocol: protocolconv.ProtocolOpenAIResponses,
...
func (s *OpenAIGatewayService) handleStructuredPassthroughSSEToJSONWithOutput(
	...
	terminal, err := s.collectOpenAICompatBufferedTerminal(resp, c, "openai passthrough buffered SSE", time.Now())
	if err != nil {
		return nil, err
	}
	...
	if strings.TrimSpace(terminal.Response.Status) == "failed" {
		...
		if compactErr := newOpenAICompactFallbackSignal(c, terminal.Upstream.Body, msg); compactErr != nil {
			return nil, compactErr
		}
		if hit, code, cyberMsg := detectOpenAICyberPolicy(terminal.Upstream.Body); hit {
			MarkOpsCyberPolicy(c, CyberPolicyMark{
				Code: code, Message: cyberMsg, Body: truncateString(string(terminal.Upstream.Body), 4096),
				UpstreamStatus: http.StatusOK, UpstreamInTok: terminal.Usage.InputTokens, UpstreamOutTok: terminal.Usage.OutputTokens,
			})
		}
	...
	terminal.Upstream.Body, err = restoreOpenAIResponsesClientToolPayload(c, terminal.Upstream.Body)
	if err != nil {
		return nil, fmt.Errorf("restore OpenAI Responses client tools: %w", err)
	}
	terminal.Upstream.Body, err = restoreOpenAIResponsesNamespacePayload(c, terminal.Upstream.Body)
	if err != nil {
		return nil, fmt.Errorf("restore OpenAI passthrough namespace response: %w", err)
	}
	terminal.Upstream.Body = restoreCodexToolNamesFromContext(c, terminal.Upstream.Body)
	if observer := upstreamResponseModelObserverFromContext(c); observer != nil {
		observer.ObserveOpenAI(terminal.Upstream.Body, "")
	}
	if isNativeResponsesProtocolOutput(output) {
...
func (s *OpenAIGatewayService) handlePassthroughSSEToJSONWithOutput(resp *http.Response, c *gin.Context, account *Account, body []byte, originalModel string, mappedModel string, output openAIProtocolOutput) (*openaiNonStreamingResultPassthrough, error) {
