...
func (s *BillingService) ResolveContextPricingSchedule(ctx context.Context, resolver *ModelPricingResolver, in ContextPricingScheduleInput) (*ContextPricingSchedule, error) {
	...
	pricingInput := PricingInput{Model: in.Model, Platform: in.Platform, Group: in.Group}
	if in.Group != nil {
		gid := in.Group.ID
	...
	if !hasAnyContextPricing(tiers) {
		return nil, tokenPricingUnavailableError(in.Model)
	}
...
func hasAnyContextPricing(tiers []ContextPricingTier) bool {
	for _, tier := range tiers {
		if tier.Input != nil || tier.Output != nil || tier.CacheWrite != nil || tier.CacheRead != nil {
			return true
		}
	}
	return false
