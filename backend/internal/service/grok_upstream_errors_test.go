...
func TestGrokContentPolicy403DoesNotMutateOrFailover(t *testing.T) {
	...
	got := svc.failoverOpenAIStructuredUpstreamError(
		context.Background(), c, account, upstream, "text is sensitive", "grok-4.5",
	)
	require.Nil(t, got)
	require.Zero(t, repo.tempUnschedCalls)
}
...
func TestGrokNonFailoverDoesNotApplyGenericTempUnschedulablePolicy(t *testing.T) {
	...
	got := svc.failoverOpenAIStructuredUpstreamError(
		context.Background(), c, account, upstream, "text is sensitive", "",
	)
...
func TestGrokContentPolicy403SharedErrorFallbackDoesNotMutate(t *testing.T) {
	...
	_, err = svc.handleCompatErrorResponse(upstream, c, account, writeChatCompletionsError, "grok-4.5")
	require.Error(t, err)
	require.Equal(t, http.StatusForbidden, recorder.Code)
	require.Contains(t, recorder.Body.String(), "invalid_request_error")
