...
func (s *BillingService) getModelPricingForPlatformAt(platform, model string, now time.Time) (*ModelPricing, error) {
	...
	for _, candidate := range billingModelPricingCandidates(model) {
		if pricing, ok := openCodeGoReferencePricingAt(candidate, now); ok {
			return s.applyModelSpecificPricingPolicyEx(candidate, pricing, false), nil
		}
	}
...
func (s *BillingService) GetModelPricing(model string) (*ModelPricing, error) {
	...
	// 先遍历全部规范名和别名候选的动态价格，再回落到内置价格。
	// 这样 OpenCode Go 别名不会被原始名称上更宽泛的家族回退提前截获。
	for _, candidate := range candidates {
...
func (s *BillingService) calculateTokenCost(resolved *ResolvedPricing, input CostInput) (*CostBreakdown, error) {
	...
	// 默认通用价卡应用 DeepSeek 官方价强制覆盖（幂等，GetModelPricing 内部已强制过）；
	// OpenCode Go 已按平台专属峰谷价选档，分组/渠道自定义定价也保留运营者配置。
	forceDeepSeekRates := resolved.Source == PricingSourceLiteLLM && !isOpenCodeGoPricingPlatform(input.PricingPlatform)
	pricing = s.applyModelSpecificPricingPolicyEx(input.Model, pricing, forceDeepSeekRates)

	// DeepSeek 通用默认价卡按官方峰谷口径调整：高峰时段（01:00–04:00 与
	// 06:00–10:00 UTC，仅工作日；北京时间周末全天低谷）按 2× 低谷价计费。
	// OpenCode Go 的平台专属目录已自行选档；分组/渠道自定义定价不叠加。
	// PricingAt 为零值时回退当前时刻。先克隆再乘，避免污染共享指针。
