...
type ModelPricing struct {
	...
	MaxReasoningEffortMultiplier       *float64           // max 推理等级的额度和计费倍率
...
func hasTokenPricingForUsage(pricing *ModelPricing, tokens UsageTokens) bool {
	...
	if tokens.InputTokens == 0 && tokens.ImageInputTokens == 0 && tokens.OutputTokens == 0 &&
...
func (s *BillingService) initFallbackPricing() {
	...
	// Gemini 3.6 Flash（公开价格：$1.50 input / $7.50 output / $0.15 cached input per MTok）。
	// Antigravity 的 high/low/medium/tiered 仅在计费层归一，不改写请求 wire ID。
	...
	// OpenAI GPT-5.6 official prices are USD/token; cache creation is 1.25x input.
...
func (s *BillingService) getModelPricingAt(model string, pricingAt time.Time) (*ModelPricing, error) {
	...
	// Prefer the live catalog unless the entry contains only image pricing or
	...
	candidates := billingModelPricingCandidates(model)
	for _, candidate := range candidates {
		if pricing, ok := s.getDynamicModelPricingExactAt(candidate, pricingAt); ok {
			return pricing, nil
		}
	}
	for _, candidate := range candidates {
		if pricing, ok := s.getFallbackModelPricingExactAt(candidate, pricingAt); ok {
			return pricing, nil
		}
	}
	return nil, fmt.Errorf("%w for model: %s", ErrModelPricingUnavailable, model)
...
func (s *BillingService) getDynamicModelPricingExactAt(model string, now time.Time) (*ModelPricing, bool) {
	if s.pricingService == nil {
		return nil, false
	}
...
func (s *BillingService) getFallbackModelPricingExactAt(model string, pricingAt time.Time) (*ModelPricing, bool) {
	...
	return s.applyModelSpecificPricingPolicyEx(model, fallback, true, pricingAt), true
}
...
func (s *BillingService) calculateTokenCost(resolved *ResolvedPricing, input CostInput) (*CostBreakdown, error) {
	...
	if forceDeepSeekRates && isDeepSeekModel(input.Model) {
...
func (s *BillingService) calculateCostForPlatformWithServiceTier(
	...
	if len(pricing.Intervals) > 0 {
		...
		if interval := FindMatchingInterval(pricing.Intervals, totalContextTokens); interval != nil {
			pricing = intervalToModelPricing(interval, pricing, nil)
