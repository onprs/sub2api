...
// recordUsageOpts 携带需要显式隔离的模型定价平台。
...
// recordUsageCoreInput 是 recordUsageCore 的公共输入字段。
...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput, opts *recordUsageOpts) error {
	...
	if apiKey.Group != nil && apiKey.Group.Platform == PlatformComposite {
		billingModel = s.compositeBillableModel(ctx, apiKey, billingModel, concreteBillingModel, opts.PricingPlatform)
	}
	...
	// response_model 只在渠道显式开启、模型可确定识别且重算不会抬价时生效。
	baselineBillingModel := chargedBillingModel
	...
	if responseModel := responseModelBillingDeclaration(
		...
		if identified, responseChannelPriced := s.hasIdentifiedResponseModelPricing(ctx, responseModel, apiKey, opts.PricingPlatform); identified {
			...
			if responseErr == nil && responseModelBillingAdoptable(cost, responseCost, baselineChannelPriced, responseChannelPriced) {
				logResponseModelBillingApplied("service.gateway", account, result.RequestID, baselineBillingModel, responseModel, cost, responseCost)
				cost = responseCost
			}
		}
	}
...
func (s *GatewayService) compositeBillableModel(ctx context.Context, apiKey *APIKey, billingModel, concreteBillingModel, pricingPlatform string) string {
	if concreteBillingModel == "" || billingModel == concreteBillingModel {
		return billingModel
	}
	if s.resolveChannelPricing(ctx, billingModel, apiKey, pricingPlatform) != nil {
		return billingModel
	}
...
func (s *GatewayService) billableModelWithFallback(ctx context.Context, apiKey *APIKey, billingModel, pricingPlatform string, fallbacks ...string) string {
	if s.hasResolvableTokenPricing(ctx, billingModel, apiKey, pricingPlatform) {
		return billingModel
	}
	for _, fallback := range fallbacks {
		...
		if s.hasResolvableTokenPricing(ctx, fallback, apiKey, pricingPlatform) {
...
func (s *GatewayService) hasResolvableTokenPricing(ctx context.Context, model string, apiKey *APIKey, pricingPlatform string) bool {
	if strings.TrimSpace(model) == "" {
		return false
	}
	if s.resolveChannelPricing(ctx, model, apiKey, pricingPlatform) != nil {
		return true
	}
	if s.billingService == nil {
		return false
	}
	_, err := s.billingService.GetModelPricingForPlatform(pricingPlatform, model)
	return err == nil
}
...
func (s *GatewayService) hasIdentifiedResponseModelPricing(
	ctx context.Context,
	model string,
	apiKey *APIKey,
	pricingPlatform string,
...
func (s *GatewayService) resolveChannelPricing(ctx context.Context, billingModel string, apiKey *APIKey, pricingPlatform string) *ResolvedPricing {
...
func (s *GatewayService) calculateTokenCost(
	...
	if s.resolver != nil && apiKey.Group != nil {
		gid := apiKey.Group.ID
		resolved = s.resolver.Resolve(ctx, PricingInput{
			Model: billingModel, GroupID: &gid, Group: apiKey.Group, Platform: opts.PricingPlatform,
		})
	}
