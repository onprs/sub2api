...
func (s *OpenAIGatewayService) forwardAnthropicViaNativeAnthropicEndpoint(
	...
	if resp.StatusCode >= 400 {
		upstream, upstreamMsg, collectErr := s.collectOpenAIStructuredUpstreamError(resp, protocolconv.ProtocolAnthropic, clientStream)
		if collectErr != nil {
		...
		// 非 failover 错误按 Anthropic 客户端契约回写。
		return s.handleAnthropicErrorResponse(upstream, c, account, billingModel)
	}
