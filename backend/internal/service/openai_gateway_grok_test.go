...
func TestForwardGrokMediaVideoGenerationDoesNotCommitBeforeOwnerBinding(t *testing.T) {
	t.Setenv(xai.EnvAllowUnsafeURLOverrides, "true")
	gin.SetMode(gin.TestMode)
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"}},
		Body:       io.NopCloser(strings.NewReader(`{"request_id":"video-request-unbound"}`)),
	}}
	cache := &stubGatewayCache{setSessionErr: errors.New("redis unavailable")}
	svc := &OpenAIGatewayService{httpUpstream: upstream, cache: cache}
	groupID := int64(7)

	result, err := svc.ForwardGrokMediaWithVideoOwner(
		context.Background(),
		c,
		account,
		...
		body,
		"application/json",
		&GrokMediaVideoRequestOwner{GroupID: &groupID, UserID: 41, APIKeyID: 51},
	)

	var bindingErr *GrokMediaVideoBindingError
	require.ErrorAs(t, err, &bindingErr)
	require.Nil(t, result)
	require.Equal(t, 1, cache.setSessionCalls)
...
func TestGrokMediaVideoRequestBindingIsScopedToUserAndAPIKey(t *testing.T) {
	...
	require.Equal(t, 1, cache.refreshSessionCalls)
...
func TestForwardGrokMedia429PoolModeSkipsRateLimit(t *testing.T) {
	t.Setenv(xai.EnvAllowUnsafeURLOverrides, "true")
	gin.SetMode(gin.TestMode)
...
func TestForwardGrokResponsesHTTPErrorUsesStructuredBoundary(t *testing.T) {
	...
	for _, stream := range []bool{false, true} {
		t.Run(map[bool]string{false: "buffered", true: "stream"}[stream], func(t *testing.T) {
			...
			require.Equal(t, "bad request", gjson.Get(recorder.Body.String(), "error.message").String())
...
func TestHandleGrokAccountUpstreamError5xxRespectsPoolMode(t *testing.T) {
	...
	t.Run("api key without pool mode gets default cooldown", func(t *testing.T) {
		...
		require.True(t, repo.lastTempUnschedUntil.After(time.Now()))
	})
...
func TestHandleGrokAccountUpstreamError429APIKeyWithoutPoolModeSetsRateLimit(t *testing.T) {
	account := &Account{ID: 616, Platform: PlatformGrok, Type: AccountTypeAPIKey}
	repo := &grokQuotaAccountRepo{}
	svc := &OpenAIGatewayService{accountRepo: repo}
	before := time.Now()
...
func TestHandleGrokAccountUpstreamErrorPoolModeSkipsDefaultRateLimits(t *testing.T) {
...
func TestUpdateGrokUsageSnapshotPoolModeExhaustedSuccessStaysSchedulable(t *testing.T) {
	account := &Account{
...
func TestFailoverOpenAIStructuredUpstreamErrorUsesOnlyGrokRateLimitPolicy(t *testing.T) {
	gin.SetMode(gin.TestMode)
	repo := &grokQuotaAccountRepo{}
	svc := &OpenAIGatewayService{accountRepo: repo}
	...
	failoverErr := svc.failoverOpenAIStructuredUpstreamError(
		context.Background(), c, account, upstream, "rate limited", "grok-4.3",
	)
