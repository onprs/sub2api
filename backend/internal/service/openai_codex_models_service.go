...
// accounts: the upstream manifest endpoint requires ChatGPT OAuth-like
...
func (s *OpenAIGatewayService) SelectCodexModelsManifestAccount(ctx context.Context, groupID *int64) (*Account, error) {
	...
	for {
		...
		if err != nil {
			if skipped > 0 {
				return nil, fmt.Errorf("no available OpenAI OAuth-like accounts for Codex models manifest after skipping %d ineligible accounts: %w", skipped, ErrNoAvailableAccounts)
			}
			return nil, err
		}
...
func isOpenAICodexImageInputModel(modelID string) bool {
	normalized := canonicalizeOpenAIModelAliasSpelling(modelID)
	return isOpenAIGPT6AstraModel(modelID) || strings.HasPrefix(normalized, "gpt-5") ||
...
func (s *OpenAIGatewayService) resolveCodexModelsManifestCredentialAccount(ctx context.Context, account *Account) (*Account, string, error) {
	...
	if account.Type != AccountTypeOAuth && account.Type != AccountTypeSetupToken {
		return nil, "non_oauth_account", fmt.Errorf("account %d type %s is not OpenAI OAuth-like", account.ID, account.Type)
	}
	...
	if credAccount == nil || !credAccount.IsOpenAIOAuthLike() {
		return nil, "not_openai_oauth", fmt.Errorf("account %d credential owner is not OpenAI OAuth-like", account.ID)
	}
...
func (s *OpenAIGatewayService) getCodexModelsManifestAccessToken(ctx context.Context, credAccount *Account) (string, error) {
	if credAccount == nil || !credAccount.IsOpenAIOAuthLike() {
		return "", errors.New("credential account is not OpenAI OAuth-like")
	}
	if credAccount.Type == AccountTypeOAuth && s != nil && s.openAITokenProvider != nil {
...
// ChatGPT backend using the account's OAuth-like credentials.
//
// After validating the stable top-level envelope, OAuth-like response bodies are
...
//
// Scope is deliberately limited to OAuth-like accounts: the manifest
