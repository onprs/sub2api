		...
		for _, stream := range []bool{false, true} {
			for _, tc := range []struct {
				...
				t.Run(provider+"/"+mode+"/"+tc.name, func(t *testing.T) {
					...
					if provider == "antigravity" && (tc.name == "minimal" || tc.name == "medium") {
					...
					wantMultiplier := tc.multiplier
					if provider == "antigravity" {
						if tc.name == "snake_case_high" {
							require.Equal(t, "high", gjson.GetBytes(upstream.requestBodies[0], "request.generation_config.thinking_config.thinking_level").String())
						} else {
							wantBudget := int64(10001)
							if tc.name == "low" {
								wantBudget = 1001
							}
							require.Equal(t, wantBudget, gjson.GetBytes(upstream.requestBodies[0], "request.generationConfig.thinkingConfig.thinkingBudget").Int())
						}
						if wantEffort == "" {
							wantEffort = "high"
							wantMultiplier = 2
					...
					require.Equal(t, wantEffort, optionalStringValue(result.ReasoningEffort))
					if wantEffort == "" {
					...
					apiKey := &APIKey{ID: 10, GroupID: &groupID, Group: &Group{
						...
						ModelPricing: []ChannelModelPricing{{
							Models: []string{result.Model}, BillingMode: BillingModeToken,
					...
					require.Equal(t, wantEffort, optionalStringValue(usageRepo.lastLog.ReasoningEffort))
					require.InDelta(t, wantMultiplier, usageRepo.lastLog.TotalCost, 1e-12)
					require.InDelta(t, wantMultiplier*0.5, usageRepo.lastLog.ActualCost, 1e-12)
					require.InDelta(t, wantMultiplier*0.5, userRepo.lastAmount, 1e-12)
				})
			}
		}
	}
