...
func TestWriteOpenAIWSPassthroughFrameWithTimeoutBoundsBackpressure(t *testing.T) {
	conn := newStagedPassthroughConn()
	for i := 0; i < cap(conn.writes); i++ {
		require.NoError(t, conn.WriteFrame(context.Background(), coderws.MessageText, []byte(`{"type":"session.update"}`)))
	}

	started := time.Now()
	err := writeOpenAIWSPassthroughFrameWithTimeout(
		context.Background(),
		conn,
		25*time.Millisecond,
		coderws.MessageText,
