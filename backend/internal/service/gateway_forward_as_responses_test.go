...
func responsesAnthropicTestPipeline(t *testing.T, stream bool) *protocolconv.Pipeline {
	return responsesAnthropicMappedTestPipeline(t, stream, "claude-sonnet-4.5", "claude-sonnet-4.5")
}

func responsesAnthropicMappedTestPipeline(t *testing.T, stream bool, clientModel, upstreamModel string) *protocolconv.Pipeline {
	t.Helper()
	pipeline, err := protocolconv.NewPipeline(standardProtocolRegistry, protocolconv.PipelineConfig{
		Route: protocolconv.Route{
			Source: protocolconv.ProtocolOpenAIResponses, IntendedTarget: protocolconv.ProtocolAnthropic,
			ClientModel: clientModel, UpstreamModel: upstreamModel, Provider: PlatformAnthropic, AccountID: 1,
		},
		Options: protocolconv.Options{SourceModel: upstreamModel, LossPolicy: protocolconv.LossError},
	})
	require.NoError(t, err)
	body, err := json.Marshal(map[string]any{"model": clientModel, "input": "hi", "stream": stream})
	require.NoError(t, err)
...
func TestOpus55ResponsesSignedThinkingBufferedAndStreamed(t *testing.T) {
	...
	for _, stream := range []bool{false, true} {
		...
		var err error
		if stream {
			_, err = svc.handleResponsesStreamingResponse(resp, c, responsesAnthropicMappedTestPipeline(t, true, "public-opus", "claude-opus-5-5"), "public-opus", "claude-opus-5-5", nil, time.Now(), apicompat.ResponsesClientToolMapping{})
		} else {
			_, err = svc.handleResponsesBufferedStreamingResponse(resp, c, responsesAnthropicMappedTestPipeline(t, false, "public-opus", "claude-opus-5-5"), "public-opus", "claude-opus-5-5", nil, time.Now(), apicompat.ResponsesClientToolMapping{})
		}
		require.NoError(t, err)
		require.Contains(t, rec.Body.String(), `"encrypted_content":"signed"`)
