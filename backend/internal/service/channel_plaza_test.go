...
func newPlazaChannelService(channels []Channel, groups []Group, pricing *PricingService) *ChannelService {
	...
	svc := NewChannelService(repo, &stubGroupRepoForAvailable{activeGroups: groups}, nil, nil, nil)
...
func TestListPlazaGroups_GroupCentricAggregation(t *testing.T) {
	...
	require.Equal(t, PricingSourceChannel, out[0].Models[0].PricingSource)
	require.Equal(t, "claude-sonnet", out[0].Models[1].Name)
	require.Equal(t, PricingSourceChannel, out[0].Models[1].PricingSource)
}
...
func TestListPlazaGroups_CompositeAndOrdinaryGroupsDoNotLeakPlatforms(t *testing.T) {
	...
	require.Equal(t, []PlazaModel{{
		Name: "claude-sonnet", Platform: PlatformAnthropic, Pricing: byName["anthropic-only"].Models[0].Pricing, PricingSource: "channel",
