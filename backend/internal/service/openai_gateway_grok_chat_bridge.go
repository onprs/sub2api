...
func (s *OpenAIGatewayService) forwardGrokChatCompletionsViaResponses(
	...
	if err != nil {
		return nil, fmt.Errorf("create grok chat responses pipeline: %w", err)
	}
	...
	if err := json.Unmarshal(convertedRequest.Body, &responsesReq); err != nil {
		return nil, fmt.Errorf("decode converted grok responses request: %w", err)
	}
	...
	if resp.StatusCode >= http.StatusBadRequest {
		...
		upstream.RequestID = firstNonEmpty(upstream.RequestID, upstream.Headers.Get("x-request-id"), upstream.Headers.Get("xai-request-id"))
		if upstreamMsg == "" {
	...
	if clientStream {
		result, err = s.handleChatStreamingResponse(resp, c, account, pipeline, originalModel, billingModel, upstreamModel, startTime, len(body))
	} else {
		result, err = s.handleChatBufferedStreamingResponse(resp, c, account, pipeline, originalModel, billingModel, upstreamModel, startTime)
	}
	if result != nil {
