...
func TestForwardGoogleGenAI_AnthropicBufferedUsesPipelineAndMappedEndpoint(t *testing.T) {
	...
	toolResult := gjson.GetBytes(upstream.lastBody, "messages.2.content.0.content")
	require.Equal(t, gjson.String, toolResult.Type)
	require.Equal(t, "ok", gjson.Get(toolResult.String(), "value").String())
