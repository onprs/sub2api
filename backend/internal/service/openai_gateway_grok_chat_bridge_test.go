...
func TestForwardGrokChatViaResponsesNonStreamingCachesAndReturnsChat(t *testing.T) {
	...
	require.Equal(t, "be concise", gjson.GetBytes(upstream.lastBody, "instructions").String())
...
func TestForwardGrokChatViaResponsesTraeToolHistoryKeepsCacheRoute(t *testing.T) {
	...
	require.Equal(t, "call_lookup", gjson.GetBytes(upstream.lastBody, "input.1.call_id").String())
	require.Equal(t, "lookup", gjson.GetBytes(upstream.lastBody, "input.1.name").String())
	require.Equal(t, `{"key":"alpha"}`, gjson.GetBytes(upstream.lastBody, "input.1.arguments").String())
	...
	require.Equal(t, `{"value":"ok"}`, gjson.GetBytes(upstream.lastBody, "input.2.output").String())
}
