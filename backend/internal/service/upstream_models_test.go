...
func TestBuildUpstreamModelsRequestSupportsOpenAISetupToken(t *testing.T) {
	svc := &AccountTestService{cfg: upstreamModelSyncTestConfig()}
	account := &Account{
		ID:       13,
		Platform: PlatformOpenAI,
		...
		Credentials: map[string]any{
			...
			"chatgpt_account_id": "chatgpt-setup-account",
		},
	}
	...
	require.NoError(t, err)
	require.Equal(t, chatgptCodexModelsURL, req.URL.Scheme+"://"+req.URL.Host+req.URL.Path)
	require.Equal(t, "Bearer openai-setup-token", req.Header.Get("Authorization"))
	require.Equal(t, "chatgpt-setup-account", req.Header.Get("chatgpt-account-id"))
}
