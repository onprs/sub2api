...
// handlePassthroughSSEToJSONWithOutput 把所有终止事件塞进固定 502，而几百行外的流式读取器对
...
func TestNonStreamingPassthroughSSEToJSON_CapacityFailedEventFailsOver(t *testing.T) {
	...
	result, err := svc.handlePassthroughSSEToJSONWithOutput(newNonStreamingSSEResponse(), c, newNonStreamingFailoverAccount(), body, "model", "model", nil)
