...
// GetUsage 获取账号使用量。
// OAuth 账号调用对应上游接口；Setup Token 按 session_window 推算；受支持的 API Key 平台走各自用量接口。
...
func (s *AccountUsageService) refreshOpenCodeGoConsoleUsageSnapshot(ctx context.Context, account *Account) (map[string]any, error) {
	...
	if s.accountRepo != nil {
		...
		mergeAccountExtra(&Account{Extra: mergedExtra}, updates)
		s.syncOpenCodeGoOfficialUsageRateLimitAt(ctx, account.ID, account.RateLimitResetAt, mergedExtra, summary.FetchedAt)
	}
	return updates, nil
}
...
func (s *AccountUsageService) syncOpenCodeGoOfficialUsageRateLimit(ctx context.Context, accountID int64, currentResetAt *time.Time, extra map[string]any) {
	s.syncOpenCodeGoOfficialUsageRateLimitAt(ctx, accountID, currentResetAt, extra, time.Now())
}

func (s *AccountUsageService) syncOpenCodeGoOfficialUsageRateLimitAt(ctx context.Context, accountID int64, currentResetAt *time.Time, extra map[string]any, now time.Time) {
...
func (s *AccountUsageService) SyncRateLimitedOpenCodeGoAccounts(ctx context.Context) {
	...
	for i := range accounts {
		acc := &accounts[i]
		if acc.IsOpenCodeGoPlan() && acc.RateLimitResetAt != nil && acc.RateLimitResetAt.After(now) && hasOpenCodeGoConsoleCredentials(acc) {
