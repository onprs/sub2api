...
func TestCallProvider_BasePath(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			_, _, status, err := callProviderWithClient(context.Background(), tc.provider,
				server.URL+tc.basePath, "test-key", "test-model", "hello", &CheckOptions{APIMode: tc.apiMode}, monitorHTTPClient)
