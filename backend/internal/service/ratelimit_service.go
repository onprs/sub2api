...
func (s *RateLimitService) HandleFirstOutputFailoverTimeout(ctx context.Context, account *Account, model string, cooldown time.Duration) bool {
	return s.setOpenAIFailoverCooldown(
		ctx,
		account,
		model,
		cooldown,
		...
		"OpenAI first-output failover timeout for model: "+model,
		"openai_first_output_timeout_cooldown",
		"first_output_timeout",
	)
}

func isOpenAIUpstreamFailoverCooldownStatus(statusCode int) bool {
	switch statusCode {
...
// HandleOpenAIUpstreamFailoverCooldown 对显式配置冷却的账号处理上游 502/503/504。
// 池模式与全局错误策略不能覆盖这项账号级配置。
func (s *RateLimitService) HandleOpenAIUpstreamFailoverCooldown(
	ctx context.Context,
	account *Account,
	model string,
	statusCode int,
) bool {
	if !isOpenAIUpstreamFailoverCooldownStatus(statusCode) {
		return false
	}
	cooldown := openAIAccountFirstOutputFailoverCooldown(account)
	if cooldown <= 0 {
		return false
	}
	return s.setOpenAIFailoverCooldown(
		...
		cooldown,
		statusCode,
		fmt.Sprintf("upstream_http_%d", statusCode),
		fmt.Sprintf("OpenAI upstream HTTP %d failover for model: %s", statusCode, model),
		"openai_upstream_failover_cooldown",
		"openai_upstream_failover",
	)
}

func (s *RateLimitService) setOpenAIFailoverCooldown(
	ctx context.Context,
	account *Account,
	model string,
	...
	s.notifyAccountSchedulingBlocked(account, until, runtimeReason)
	if err := s.accountRepo.SetTempUnschedulable(ctx, account.ID, until, reason); err != nil {
		slog.Warn(logEventPrefix+"_set_temp_unsched_failed", "account_id", account.ID, "error", err)
		return false
	}
	if s.tempUnschedCache != nil {
		if err := s.tempUnschedCache.SetTempUnsched(ctx, account.ID, state); err != nil {
			slog.Warn(logEventPrefix+"_set_temp_unsched_cache_failed", "account_id", account.ID, "error", err)
		}
	}

	slog.Info(logEventPrefix+"_temp_unschedulable", "account_id", account.ID, "until", until, "model", model, "status_code", statusCode)
	return true
}
