...
func TestOpenAIGatewayServiceForwardOAuthMarkedNativeCompactV2InjectsFeature(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	svc := &OpenAIGatewayService{cfg: cfg, httpUpstream: upstream}
	account := &Account{
		ID: 12, Name: "openai-oauth-responses", Platform: PlatformOpenAI,
		Type: AccountTypeOAuth, Concurrency: 1, Status: StatusActive, Schedulable: true,
		Credentials: map[string]any{
...
func TestOpenAIGatewayServiceForwardAPIKeyMarkedNativeCompactV2MergesFeature(t *testing.T) {
	gin.SetMode(gin.TestMode)
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}},
	...
	svc := &OpenAIGatewayService{cfg: cfg, httpUpstream: upstream}
	account := &Account{
		ID: 11, Name: "openai-apikey-responses", Platform: PlatformOpenAI,
	...
	require.Equal(t, "compaction_trigger", gjson.GetBytes(upstream.lastBody, "input.#(type==\"compaction_trigger\").type").String())
	require.Equal(t, "max", gjson.GetBytes(upstream.lastBody, "reasoning.effort").String())
	require.Contains(t, upstream.lastReq.Header.Get("x-codex-beta-features"), "some_other_feature")
