...
func TestLoadEmailOAuthProxyConfigDefaultsRemainUnset(t *testing.T) {
	resetViperWithJWTSecret(t)

	cfg, err := Load()
	require.NoError(t, err)
	require.Nil(t, cfg.GitHubOAuth.ProxyID)
	require.Empty(t, cfg.GitHubOAuth.ProxyURL)
	require.Nil(t, cfg.GoogleOAuth.ProxyID)
	require.Empty(t, cfg.GoogleOAuth.ProxyURL)
}

func TestLoadEmailOAuthProxyConfigFromEnvironment(t *testing.T) {
	resetViperWithJWTSecret(t)
	t.Setenv("GITHUB_OAUTH_PROXY_ID", "7")
	t.Setenv("GITHUB_OAUTH_PROXY_URL", "http://127.0.0.1:7890")
	t.Setenv("GOOGLE_OAUTH_PROXY_ID", "8")
	t.Setenv("GOOGLE_OAUTH_PROXY_URL", "socks5://127.0.0.1:1080")

	cfg, err := Load()
	require.NoError(t, err)
	require.NotNil(t, cfg.GitHubOAuth.ProxyID)
	require.Equal(t, int64(7), *cfg.GitHubOAuth.ProxyID)
	require.Equal(t, "http://127.0.0.1:7890", cfg.GitHubOAuth.ProxyURL)
	require.NotNil(t, cfg.GoogleOAuth.ProxyID)
	require.Equal(t, int64(8), *cfg.GoogleOAuth.ProxyID)
	require.Equal(t, "socks5://127.0.0.1:1080", cfg.GoogleOAuth.ProxyURL)
}
