...
type ServerConfig struct {
	...
	PublicReadTimeoutSeconds int       `mapstructure:"public_read_timeout_seconds"` // 公开请求体读取超时（秒）
...
func load(allowMissingJWTSecret bool) (*Config, error) {
	...
	} else if cfg.Security.URLAllowlist.AllowPrivateHosts {
		slog.Warn("security.url_allowlist.allow_private_hosts=true; final private-IP dial guard is disabled for allowlisted upstreams.")
	}
...
func setDefaults() {
	...
	viper.SetDefault("server.public_read_timeout_seconds", 30) // 30秒读取公开请求体
...
func setEnvReachableDefaults() {
	...
	for _, provider := range []string{"github_oauth", "google_oauth"} {
		...
		// proxy_id 必须保持 nil 以区分“未配置”和编号 0，因此用 BindEnv
		// 注册可达性；proxy_url 同样显式绑定，避免纯环境变量部署静默丢弃。
		prefix := strings.ToUpper(provider)
		_ = viper.BindEnv(provider+".proxy_id", prefix+"_PROXY_ID")
		_ = viper.BindEnv(provider+".proxy_url", prefix+"_PROXY_URL")
	}
...
func (c *Config) Validate() error {
	...
	if c.Server.PublicReadTimeoutSeconds <= 0 {
		return fmt.Errorf("server.public_read_timeout_seconds must be positive")
	}
