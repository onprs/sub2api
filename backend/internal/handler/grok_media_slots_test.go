...
func newGrokMediaSlotHandler(t *testing.T, oauth, mismatch bool, platforms ...string) (*OpenAIGatewayHandler, *grokMediaSlotsCache, *grokMediaSlotBindings, *grokMediaSlotUpstream) {
	...
	handler := NewOpenAIGatewayHandler(gateway, nil, concurrency, billing, allowBillingEligibilityResolver{}, service.NewAPIKeyService(nil, nil, nil, nil, nil, nil, cfg), nil, nil, nil, nil, cfg)
...
func TestGrokMediaLookupSlotLifecycle(t *testing.T) {
	for _, scenario := range []string{"normal", "mismatch acquired", "mismatch wait", "full", "queue full", "cancel while waiting", "wait then acquired", "upstream error", "cancel", "panic"} {
		t.Run(scenario, func(t *testing.T) {
			...
			wantBindingWrites := 20
			if scenario == "cancel" {
				wantBindingWrites = 21
			} else if scenario == "normal" || scenario == "wait then acquired" || scenario == "upstream error" || scenario == "panic" {
				wantBindingWrites = 40
			}
			require.Equal(t, wantBindingWrites, bindings.writes, "lookups must preserve owner and renew its TTL")
