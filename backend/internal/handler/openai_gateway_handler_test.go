...
func TestOpenAIResponsesWebSocket_PassthroughTracksModelPerTurn(t *testing.T) {
	...
	require.InDelta(t, got.logs[1].TotalCost*2.5, got.logs[0].TotalCost, 1e-12,
		"each turn must be billed with its own channel-mapped model using current GPT-5.6 fallback prices")
}
...
func TestOpenAIResponsesWebSocket_PassthroughKeepsTurnMappingSnapshot(t *testing.T) {
	...
	require.InDelta(t, got.logs[1].TotalCost*2.5, got.logs[0].TotalCost, 1e-12,
		"the next turn must use the updated channel mapping with current GPT-5.6 fallback prices")
}
...
func (s *openAIWSFailoverHandlerAccountRepoStub) ListModelAvailabilityCandidates(_ context.Context, groupID *int64, platforms []string, includeGrouped bool) ([]service.Account, error) {
	...
	out := make([]service.Account, 0, len(s.accounts))
	for _, account := range s.accounts {
		if _, ok := allowedPlatforms[account.Platform]; !ok || account.Status != service.StatusActive || !account.Schedulable {
			continue
		}
		if groupID == nil {
			if !includeGrouped && len(account.AccountGroups) > 0 {
				continue
			}
			out = append(out, account)
			continue
...
func (openAIWSUsageHandlerBillingRepoStub) Apply(context.Context, *service.UsageBillingCommand) (*service.UsageBillingApplyResult, error) {
...
func TestOpenAIResponses_APIKeyPassthroughSSERateLimitUsesConfiguredPoolRetry(t *testing.T) {
	...
	accounts := []service.Account{
		{
			...
			AccountGroups: []service.AccountGroup{{AccountID: 9912, GroupID: groupID}},
	...
	// 池模式同账号重试一次（9912×2）；重试耗尽后按 maxAccountSwitches 切换到
	// 备选账号 9911。合并后 simple 模式不校验分组归属，9911（无分组）可用。
	require.Equal(t, []int64{9912, 9912, 9911}, upstream.calls())
...
func TestOpenAIResponses_APIKeyPassthroughSSERateLimitWithoutFallbackPreservesRetryAfter(t *testing.T) {
	gin.SetMode(gin.TestMode)
	groupID := int64(4204)
	accounts := []service.Account{
		{
...
func TestOpenAIResponsesWebSocket_FailoverOnUpstreamUsageLimitEvent(t *testing.T) {
	...
	require.Equal(t, []bool{true}, routingCache.routingOutcomes(groupID))
}
...
func runOpenAIResponsesWebSocketUsageLogCase(t *testing.T, tc openAIResponsesWSUsageLogCase) openAIResponsesWSUsageLogResult {
	...
	if tc.simpleModeRejectAtRead > 0 {
		usageBillingRepo = openAIWSUsageHandlerBillingRepoStub{}
	}
	gatewaySvc := service.NewOpenAIGatewayService(
		accountRepo,
		usageRepo,
