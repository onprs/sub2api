...
func (h *OpenAIGatewayHandler) handleGrokMedia(c *gin.Context, endpoint service.GrokMediaEndpoint, requestID string) {
	...
	for {
		...
		result, err := func() (*service.OpenAIForwardResult, error) {
			...
			return h.gatewayService.ForwardGrokMediaWithVideoOwner(
				requestCtx,
				c,
				account,
				endpoint,
				...
				body,
				contentType,
				&service.GrokMediaVideoRequestOwner{
					GroupID:  apiKey.GroupID,
		...
		if err != nil {
			var bindingErr *service.GrokMediaVideoBindingError
			if errors.As(err, &bindingErr) {
				...
				if !service.IsResponseCommitted(c) && c.Writer.Size() == writerSizeBeforeForward {
					h.errorResponse(c, http.StatusServiceUnavailable, "grok_media_binding_unavailable", "Video request tracking is temporarily unavailable")
				}
		...
		if endpoint.IsGenerationRequest() && strings.TrimSpace(result.ResponseID) != "" {
			if apiKey.UsesDynamicGroupRouting() && apiKey.GroupID != nil && h.routingService != nil {
				h.routingService.BindAPIKeyRoutingSession(
					requestCtx,
					apiKey.ID,
					service.GrokMediaVideoRequestSessionHash(result.ResponseID, subject.UserID, apiKey.ID),
					*apiKey.GroupID,
				)
			}
		}
