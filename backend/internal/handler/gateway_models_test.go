...
func TestGatewayModels_GoogleDiscoveryRendersOpenAIGroupModels(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	apiKey := apiKeyWithGroupForModelsTest(groupID, service.PlatformOpenAI)
	...
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &got))
	require.Equal(t, []string{"models/gpt-5.4"}, geminiModelNamesForTest(got.Models))
	require.Equal(t, []string{"generateContent", "streamGenerateContent"}, got.Models[0].SupportedGenerationMethods)
}

func TestGatewayModels_GoogleDiscoveryGetsOnlyRoutableOpenAIModel(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	t.Run("listed model", func(t *testing.T) {
		...
		require.Equal(t, http.StatusOK, rec.Code)
		var got gatewayGeminiModelItemForTest
		require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &got))
		require.Equal(t, "models/gpt-5.4", got.Name)
	})

	t.Run("unlisted model", func(t *testing.T) {
		rec := httptest.NewRecorder()
		c, _ := gin.CreateTestContext(rec)
		c.Request = httptest.NewRequest(http.MethodGet, "/v1beta/models/gpt-unavailable", nil)
		c.Params = gin.Params{{Key: "model", Value: "gpt-unavailable"}}
		c.Set(string(middleware2.ContextKeyAPIKey), apiKey)

		h.GeminiV1BetaGetModel(c)

		require.Equal(t, http.StatusNotFound, rec.Code)
		require.JSONEq(t, `{"error":{"code":404,"message":"Model not found: gpt-unavailable","status":"NOT_FOUND"}}`, rec.Body.String())
	})
}
