...
func (h *GatewayHandler) GeminiV1BetaListModels(c *gin.Context) {
	...
	if forcePlatform == service.PlatformAntigravity && h.gatewayService != nil {
		h.writeAntigravityGeminiMappedModels(c, apiKeyGroupIDFromContext(c))
		return
	}
...
func (h *GatewayHandler) standardGoogleIngressModelAvailable(c *gin.Context, groupID *int64, platform, model string) bool {
	...
	if apiKey, ok := middleware.GetAPIKeyFromContext(c); ok && apiKey != nil && apiKey.Group != nil && apiKey.Group.ModelAllowlistEnabled() {
		return false
	}
...
func (h *GatewayHandler) standardGoogleIngressModelIDs(c *gin.Context, groupID *int64, platform string) []string {
	...
	if apiKey, ok := middleware.GetAPIKeyFromContext(c); ok && apiKey != nil && apiKey.Group != nil && apiKey.Group.ModelAllowlistEnabled() {
		// 使用原始映射模式作为来源，确保白名单中的非默认模型也能被通配符映射放行。
		models = apiKey.Group.ModelAllowlist.FilterForListing(modelListingSource(platform, availableModels, fallbackModels))
	}
	return models
}
...
func (h *GatewayHandler) writeAntigravityGeminiMappedModels(c *gin.Context, groupID *int64) {
	...
	if apiKey, ok := middleware.GetAPIKeyFromContext(c); ok && apiKey != nil && !apiKey.UsesDynamicGroupRouting() && apiKey.Group != nil && apiKey.Group.ModelAllowlistEnabled() {
...
func (h *GatewayHandler) GeminiV1BetaModels(c *gin.Context) {
	...
	// Resolve the authoritative subscription after the user wait.
	...
	if subscription, err = resolveLatestBillingEligibility(c, h.billingEligibilityService, apiKey, authSubject.UserID, service.QuotaPlatform(c.Request.Context(), apiKey)); err != nil {
	...
	for {
		...
		if shouldUseAntigravityStandardBridge(account) {
...
func googleError(c *gin.Context, status int, message string) {
	writeProtocolError(c, protocolconv.ProtocolGoogleGenAI, status, "api_error", "api_error", message)
}
