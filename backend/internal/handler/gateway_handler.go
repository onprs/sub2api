...
const (
	...
	antigravityClaudeModelsMappingMessage  = "No Claude-compatible Antigravity model_mapping configured. Please ask the administrator to configure model mappings."
)
	...
	if platform == service.PlatformGemini {
		...
		for {
			...
			if account.Platform == service.PlatformAntigravity && account.Type != service.AccountTypeAPIKey {
				// The ingress body is Anthropic Messages. Forward handles both the
				// Antigravity vendor conversion and upstream-relay passthrough.
...
func (h *GatewayHandler) Models(c *gin.Context) {
	...
	if platform == service.PlatformOpenAI && apiKey != nil && !apiKey.UsesDynamicGroupRouting() && apiKey.Group != nil &&
	...
	if platform == service.PlatformCommandCode {
		writeModelsList(c, platform, service.CommandCodeDefaultModelIDs())
		return
	}
	...
	if platform == service.PlatformOpenRouter {
		writeModelsList(c, platform, service.OpenRouterDefaultModelIDs())
		return
	}
...
func (h *GatewayHandler) AntigravityModels(c *gin.Context) {
	...
	if len(modelIDs) > 0 {
		writeModelsList(c, service.PlatformAntigravity, modelIDs)
		return
	}
	c.JSON(http.StatusOK, gin.H{
		"object":  "list",
...
func (h *GatewayHandler) handleStreamingAwareErrorWithCode(c *gin.Context, status int, errType, code, message string, streamStarted bool) {
	if streamStarted {
		...
		writeProtocolStreamError(c, protocolForInboundEndpoint(c), status, errType, code, message)
		return
	}
...
func (h *GatewayHandler) errorResponseWithCode(c *gin.Context, status int, errType, code, message string) {
	writeProtocolError(c, protocolconv.ProtocolAnthropic, status, errType, code, message)
}
