...
func logOpsRecoveredUpstream(c *gin.Context, ops *service.OpsService, finalStatus int) {
	...
	clientDisconnected := c.Request != nil && c.Request.Context().Err() != nil
	...
	if lastStage == string(service.GatewayFailureStageAccountAuth) {
		...
		if clientDisconnected {
			entry.ErrorMessage = "Upstream error"
		}
		if lastStatus > 0 {
	...
	if clientDisconnected && lastStage != string(service.GatewayFailureStageAccountAuth) {
		entry.ErrorMessage += " (client disconnected)"
	}
