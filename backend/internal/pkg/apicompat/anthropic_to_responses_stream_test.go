...
func TestAnthropicEventToResponses_MultipleTextBlocksUseDistinctContentIndexes(t *testing.T) {
	state := NewAnthropicEventToResponsesState()
	...
	for _, event := range events {
		...
		case "response.content_part.added", "response.output_text.delta", "response.output_text.done", "response.content_part.done":
			indexesByType[event.Type] = append(indexesByType[event.Type], event.ContentIndex)
		}
	}
	for _, eventType := range []string{
		...
		if got := indexesByType[eventType]; len(got) != 2 || got[0] != 0 || got[1] != 1 {
			t.Fatalf("%s content indexes = %v, want [0 1]", eventType, got)
		}
	}
}
