...
func (s *AccountRepoSuite) TestListWithFilters_SortByCurrentUpstreamBillingRateDuringPeak() {
	...
	receivedAt := now.UTC().Add(-time.Minute)
	freshUntil := now.UTC().Add(time.Hour)
...
func (s *AccountRepoSuite) TestListWithFilters_UpstreamBillingRateIgnoresStaleAndNegativeSnapshots() {
	now := time.Now().UTC()
	makeAccount := func(name string, rate float64, receivedAt, freshUntil time.Time) {
		mustCreateAccount(s.T(), s.client, &service.Account{
			Name: name,
			Extra: map[string]any{
				service.UpstreamBillingProbeExtraKey: map[string]any{
	...
	makeAccount("valid-rate", 0.5, now.Add(-time.Minute), now.Add(time.Hour))
	makeAccount("stale-rate", 0.01, now.Add(-2*time.Hour), now.Add(-time.Hour))
	makeAccount("negative-rate", -1, now.Add(-time.Minute), now.Add(time.Hour))

	for _, tc := range []struct {
		order string
		want  []string
	}{
		{order: "asc", want: []string{"valid-rate", "stale-rate", "negative-rate"}},
		{order: "desc", want: []string{"valid-rate", "negative-rate", "stale-rate"}},
	} {
		accounts, _, err := s.repo.ListWithFilters(s.ctx, pagination.PaginationParams{
			Page: 1, PageSize: 10, SortBy: "upstream_billing_rate", SortOrder: tc.order,
		}, "", "", "", "", 0, "")
