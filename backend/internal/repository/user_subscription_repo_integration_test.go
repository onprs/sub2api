...
func (s *UserSubscriptionRepoSuite) TestRenewTermExpiredResetsUsageAndWindows() {
	...
	s.Require().Equal(service.InitialSubscriptionDailyWindowStart(now, renewed.ExpiresAt), *renewed.DailyWindowStart)
	s.Require().Equal(now, *renewed.WeeklyWindowStart)
	s.Require().Equal(now, *renewed.MonthlyWindowStart)
