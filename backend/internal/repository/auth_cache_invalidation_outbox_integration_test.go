...
func TestAuthCacheInvalidationTriggers_CoverSecurityMutationsOnly(t *testing.T) {
	...
	secondaryGroup := mustCreateGroup(t, integrationEntClient, &service.Group{
		Name: fmt.Sprintf("auth-outbox-secondary-%d", suffix), RateMultiplier: 1, IsExclusive: true,
	})
	...
	t.Cleanup(func() {
		...
		_, err := integrationDB.ExecContext(ctx,
			"DELETE FROM user_allowed_groups WHERE user_id = $1 OR group_id IN ($2, $3)", user.ID, group.ID, secondaryGroup.ID)
		require.NoError(t, err)
		...
		require.NoError(t, err)
		_, err = integrationDB.ExecContext(ctx, "DELETE FROM groups WHERE id IN ($1, $2)", group.ID, secondaryGroup.ID)
		require.NoError(t, err)
	})
	...
	clear()
	_, err = integrationDB.ExecContext(ctx, "UPDATE groups SET allow_live = NOT allow_live WHERE id = $1", group.ID)
	require.NoError(t, err)
	require.Equal(t, 1, count(), "Live permission changes must enqueue bound keys")
	clear()
	...
	_, err = integrationDB.ExecContext(ctx,
		"UPDATE api_keys SET routing_platform = 'openai', routing_strategy = 'balanced' WHERE id = $1", key.ID)
	require.NoError(t, err)
	require.Equal(t, 1, count(), "dynamic routing field changes must enqueue")
	clear()

	_, err = integrationDB.ExecContext(ctx,
		"INSERT INTO api_key_groups (api_key_id, group_id, priority) VALUES ($1, $2, 10)", key.ID, secondaryGroup.ID)
	require.NoError(t, err)
	require.Equal(t, 1, count(), "adding a secondary routing group must enqueue")
	clear()

	_, err = integrationDB.ExecContext(ctx,
		"UPDATE api_key_groups SET priority = 20 WHERE api_key_id = $1 AND group_id = $2", key.ID, secondaryGroup.ID)
	require.NoError(t, err)
	require.Equal(t, 1, count(), "secondary routing priority changes must enqueue")
	clear()

	_, err = integrationDB.ExecContext(ctx, "UPDATE groups SET status = 'disabled' WHERE id = $1", secondaryGroup.ID)
	require.NoError(t, err)
	require.Equal(t, 1, count(), "secondary group disable must enqueue configured keys")
	clear()
	...
	require.NoError(t, err)
	require.Equal(t, 1, count(), "secondary-group authorization grants must enqueue")
	clear()
	_, err = integrationDB.ExecContext(ctx,
		"DELETE FROM user_allowed_groups WHERE user_id = $1 AND group_id = $2", user.ID, secondaryGroup.ID)
	require.NoError(t, err)
	require.Equal(t, 1, count(), "secondary-group authorization revocations must enqueue")
	clear()

	_, err = integrationDB.ExecContext(ctx,
		"DELETE FROM api_key_groups WHERE api_key_id = $1 AND group_id = $2", key.ID, secondaryGroup.ID)
	require.NoError(t, err)
	require.Equal(t, 1, count(), "removing a secondary routing group must enqueue")
	clear()
