	...
	result, err := client.ExecContext(ctx, `
		...
		SET
			...
			extra = CASE
				...
				-- OpenCode 平台账号不在 Ollama 白名单内，与 Ollama 分支真互斥。
				WHEN type = 'apikey'
...
// ClearOpenAIRateLimitIfObserved 清除官方额度探针所观察到的 OpenAI OAuth 限流代次。
// 时间戳不匹配表示期间已有新 429，旧探针不能覆盖较新的持久化阻断。
func (r *accountRepository) ClearOpenAIRateLimitIfObserved(ctx context.Context, id int64, observedLimitedAt, observedResetAt time.Time, observedOverloadUntil *time.Time) (bool, error) {
	...
	if observedOverloadUntil == nil {
		builder.Where(dbaccount.OverloadUntilIsNil())
	} else {
		builder.Where(dbaccount.OverloadUntilEQ(*observedOverloadUntil))
	}
	updated, err := builder.
		ClearRateLimitedAt().
		ClearRateLimitResetAt().
...
func (r *accountRepository) BulkUpdate(ctx context.Context, ids []int64, updates service.AccountBulkUpdate) (int64, error) {
	...
	// OpenCode 平台的 Go 模式与挂载账号 + 官方 OpenCode Go 基址均可持有用量快照；Zen 模式排除。
	...
	if _, ok := updates.Credentials["base_url"]; ok {
		// 仅挂载行身份钉在官方基址上（OpenCode Go 模式账号的 base_url 不参与资格，两个
	...
	if len(updates.Extra) > 0 || len(ollamaGroupIdentityChanges) > 0 || len(opencodeGroupIdentityChanges) > 0 || ollamaProxyIdentityChanged != "" || updates.EnsureCodexFingerprintSeed {
		...
		// 谓词；OpenCode 平台账号不在 Ollama 白名单内，与 Ollama 分支真互斥。
