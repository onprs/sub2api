...
// 参数切片和类型表保持一致：session_id 位于 native_compaction_v2 之前，
// dynamic_rate_multiplier 作为最后一个可空快照字段。
func TestPrepareUsageLogInsert_SessionIDArgWiring(t *testing.T) {
	require.Len(t, usageLogInsertArgTypes, 64, "参数类型表必须包含上游响应审计、请求推理强度、缓存写入推断、原生压缩、upstream_request_id、session_id 和动态倍率快照")
	...
	// 末尾字段依次为 session_id、native_compaction_v2、created_at、dynamic_rate_multiplier。
	...
	require.Equal(t, "numeric", usageLogInsertArgTypes[len(usageLogInsertArgTypes)-1],
		"dynamic_rate_multiplier arg type must be numeric")
}
...
func TestUsageLogInsertQueries_IncludeSessionID(t *testing.T) {
	...
	require.Contains(t, usageLogSelectColumns, "dynamic_rate_multiplier",
		"SELECT column list must include dynamic_rate_multiplier")
	...
	require.Contains(t, batchQuery, "dynamic_rate_multiplier")
	...
	require.Contains(t, bestEffortQuery, "dynamic_rate_multiplier")
