...
func buildUserRequestHistoryCTE(filter service.UserRequestHistoryFilter) (string, []any) {
	...
	SELECT
		...
		jsonb_build_object(
			...
			'upstream_model', NULLIF(TRIM(ul.upstream_model), ''),
			'upstream_response_model', NULLIF(TRIM(ul.upstream_response_model), ''),
			'upstream_model_mismatch', ul.upstream_model_mismatch,
			...
			'dynamic_rate_multiplier', ul.dynamic_rate_multiplier,
	...
	SELECT
		...
		jsonb_build_object(
			...
			'upstream_model', NULLIF(TRIM(e.upstream_model), ''),
