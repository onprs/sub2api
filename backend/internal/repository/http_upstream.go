...
func (s *httpUpstreamService) Do(req *http.Request, proxyURL string, accountID int64, accountConcurrency int) (*http.Response, error) {
	...
	// servertiming.Do 仅在启用时采集 Server-Timing 指标，内部仍使用同一 client。
...
func (s *httpUpstreamService) DoWithTLS(req *http.Request, proxyURL string, accountID int64, accountConcurrency int, profile *tlsfingerprint.Profile) (*http.Response, error) {
	...
	proxyInfo := "direct"
	if proxyURL != "" {
		proxyInfo = service.RedactProxyURL(proxyURL)
	}
...
func (s *httpUpstreamService) getClientEntryWithTLS(proxyURL string, accountID int64, accountConcurrency int, profile *tlsfingerprint.Profile, upstreamProfile service.HTTPUpstreamProfile, markInFlight bool, enforceLimit bool) (*upstreamClientEntry, error) {
	...
	logProxyKey := redactProxyKeyForLog(proxyKey)
	logCacheKey := "tls:" + buildCacheKey(isolation, logProxyKey, accountID, upstreamProtocolModeDefault)
	...
	// 创建带 TLS 指纹的 Transport。严格模式直连时把最终 IP 复核注入底层拨号器；
	// 代理模式由管理员配置的代理负责目标 DNS 和连接。
	slog.Debug("tls_fingerprint_creating_new_client", "account_id", accountID, "cache_key", logCacheKey, "proxy", logProxyKey)
	var directDialContext func(context.Context, string, string) (net.Conn, error)
	if parsedProxy == nil && s.shouldValidateResolvedIP() {
		directDialContext = service.NewSSRFSafeDialContext(nil)
	}
	transport, err := buildUpstreamTransportWithTLSFingerprint(settings, parsedProxy, profile, directDialContext)
	if err != nil {
		s.mu.Unlock()
...
func (s *httpUpstreamService) getClientEntry(proxyURL string, accountID int64, accountConcurrency int, profile service.HTTPUpstreamProfile, markInFlight bool, enforceLimit bool) (*upstreamClientEntry, error) {
	...
	if parsedProxy == nil && s.shouldValidateResolvedIP() {
		transport.DialContext = service.NewSSRFSafeDialContext(nil)
	}
	client := &http.Client{Transport: transport}
	if s.shouldValidateResolvedIP() {
		client.CheckRedirect = s.redirectChecker
...
func (s *httpUpstreamService) recordOpenAIHTTP2Failure(profile service.HTTPUpstreamProfile, protocolMode, proxyKey string, err error) {
	...
	if activated {
		slog.Warn("openai_http2_proxy_fallback_activated",
			"proxy", redactProxyKeyForLog(proxyKey),
...
func redactProxyKeyForLog(proxyKey string) string {
	if proxyKey == directProxyKey {
		return directProxyKey
	}
	return service.RedactProxyURL(proxyKey)
}
