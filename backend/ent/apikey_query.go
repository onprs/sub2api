...
// QueryRoutingGroups chains the current query on the "routing_groups" edge.
func (_q *APIKeyQuery) QueryRoutingGroups() *GroupQuery {
	query := (&GroupClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *APIKeyQuery) Clone() *APIKeyQuery {
	...
	return &APIKeyQuery{
		...
		withRoutingGroups: _q.withRoutingGroups.Clone(),
...
// WithRoutingGroups tells the query-builder to eager-load the nodes that are connected to
// the "routing_groups" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *APIKeyQuery) WithRoutingGroups(opts ...func(*GroupQuery)) *APIKeyQuery {
	query := (&GroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withRoutingGroups = query
	return _q
}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	...
	if query := _q.withRoutingGroups; query != nil {
		if err := _q.loadRoutingGroups(ctx, query, nodes,
			func(n *APIKey) { n.Edges.RoutingGroups = []*Group{} },
			func(n *APIKey, e *Group) { n.Edges.RoutingGroups = append(n.Edges.RoutingGroups, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *APIKeyQuery) loadRoutingGroups(ctx context.Context, query *GroupQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *Group)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(apikey.RoutingGroupsTable)
		s.Join(joinT).On(s.C(group.FieldID), joinT.C(apikey.RoutingGroupsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(apikey.RoutingGroupsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(apikey.RoutingGroupsPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "routing_groups" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
