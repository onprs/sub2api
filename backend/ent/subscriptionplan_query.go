...
func (_q *SubscriptionPlanQuery) loadRedeemCodes(ctx context.Context, query *RedeemCodeQuery, nodes []*SubscriptionPlan, init func(*SubscriptionPlan), assign func(*SubscriptionPlan, *RedeemCode)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(redeemcode.FieldSubscriptionPlanID)
	}
	...
	for _, n := range neighbors {
		fk := n.SubscriptionPlanID
		if fk == nil {
			return fmt.Errorf(`foreign-key "subscription_plan_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "subscription_plan_id" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
