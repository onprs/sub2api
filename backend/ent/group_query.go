...
type GroupQuery struct {
	...
	withRoutingAPIKeys    *APIKeyQuery
...
// QueryRoutingAPIKeys chains the current query on the "routing_api_keys" edge.
func (_q *GroupQuery) QueryRoutingAPIKeys() *APIKeyQuery {
	query := (&APIKeyClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withRoutingAPIKeys:    _q.withRoutingAPIKeys.Clone(),
...
// WithRoutingAPIKeys tells the query-builder to eager-load the nodes that are connected to
// the "routing_api_keys" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *GroupQuery) WithRoutingAPIKeys(opts ...func(*APIKeyQuery)) *GroupQuery {
	query := (&APIKeyClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withRoutingAPIKeys = query
	return _q
}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withRoutingAPIKeys; query != nil {
		if err := _q.loadRoutingAPIKeys(ctx, query, nodes,
			func(n *Group) { n.Edges.RoutingAPIKeys = []*APIKey{} },
			func(n *Group, e *APIKey) { n.Edges.RoutingAPIKeys = append(n.Edges.RoutingAPIKeys, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *GroupQuery) loadRoutingAPIKeys(ctx context.Context, query *APIKeyQuery, nodes []*Group, init func(*Group), assign func(*Group, *APIKey)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(group.RoutingAPIKeysTable)
		s.Join(joinT).On(s.C(apikey.FieldID), joinT.C(group.RoutingAPIKeysPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(group.RoutingAPIKeysPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(group.RoutingAPIKeysPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "routing_api_keys" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
