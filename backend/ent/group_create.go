...
func (_c *GroupCreate) AddRoutingAPIKeyIDs(ids ...int64) *GroupCreate {
	_c.mutation.AddRoutingAPIKeyIDs(ids...)
	return _c
}

// AddRoutingAPIKeys adds the "routing_api_keys" edges to the APIKey entity.
func (_c *GroupCreate) AddRoutingAPIKeys(v ...*APIKey) *GroupCreate {
	ids := make([]int64, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
	return _c.AddRoutingAPIKeyIDs(ids...)
}
...
func (_c *GroupCreate) AddChannelMonitorIDs(ids ...int64) *GroupCreate {
	_c.mutation.AddChannelMonitorIDs(ids...)
	return _c
}

// AddChannelMonitors adds the "channel_monitors" edges to the ChannelMonitor entity.
func (_c *GroupCreate) AddChannelMonitors(v ...*ChannelMonitor) *GroupCreate {
	ids := make([]int64, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
	return _c.AddChannelMonitorIDs(ids...)
}
...
func (_c *GroupCreate) createSpec() (*Group, *sqlgraph.CreateSpec) {
	...
	if nodes := _c.mutation.RoutingAPIKeysIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
	...
	if nodes := _c.mutation.ChannelMonitorsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
