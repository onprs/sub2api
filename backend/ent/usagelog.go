...
func (*UsageLog) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case usagelog.FieldImageSizeBreakdown:
			values[i] = new([]byte)
		case usagelog.FieldUpstreamModelMismatch, usagelog.FieldCacheWriteInferred, usagelog.FieldLongContextBillingApplied, usagelog.FieldStream, usagelog.FieldCacheTTLOverridden:
			values[i] = new(sql.NullBool)
		case usagelog.FieldInputCost, usagelog.FieldOutputCost, usagelog.FieldCacheCreationCost, usagelog.FieldCacheReadCost, usagelog.FieldTotalCost, usagelog.FieldActualCost, usagelog.FieldRateMultiplier, usagelog.FieldDynamicRateMultiplier, usagelog.FieldAccountRateMultiplier:
...
func (_m *UsageLog) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case usagelog.FieldDynamicRateMultiplier:
			if value, ok := values[i].(*sql.NullFloat64); !ok {
				return fmt.Errorf("unexpected type %T for field dynamic_rate_multiplier", values[i])
			} else if value.Valid {
				_m.DynamicRateMultiplier = new(float64)
				*_m.DynamicRateMultiplier = value.Float64
			}
...
func (_m *UsageLog) String() string {
	...
	builder.WriteString(", ")
	if v := _m.DynamicRateMultiplier; v != nil {
		builder.WriteString("dynamic_rate_multiplier=")
		builder.WriteString(fmt.Sprintf("%v", *v))
	}
	builder.WriteString(", ")
