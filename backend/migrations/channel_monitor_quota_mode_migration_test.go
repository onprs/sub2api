...
func TestChannelMonitorQuotaModeMigration(t *testing.T) {
	...
	// provider CHECK 两张表扩到完整平台并集，且带幂等守卫。
	...
	require.Contains(t, sql, "CHECK (provider IN ('openai', 'anthropic', 'gemini', 'grok', 'opencode_go', 'clinepass', 'openrouter', 'commandcode', 'antigravity', 'antigravity_claude', 'antigravity_gemini', 'kimi', 'zhipu', 'deepseek'))")
