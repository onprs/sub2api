...
- Channel Monitor: use provider `clinepass` and Chat Completions mode. For an end-to-end check, use the current Sub2API `/v1` endpoint with a local ClinePass group key. Direct checks against `https://api.cline.bot/api/v1` require a Cline-issued API key. Both paths perform a real generation and consume ClinePass quota.
