...
- `security.url_allowlist.allow_anthropic_apikey_custom_hosts` to allow Anthropic API-key accounts to use arbitrary public HTTPS relays (default true; set false to keep upstream_hosts as a strict boundary for their base URLs too)
...
- Channel Monitor: use provider `clinepass` and Chat Completions mode. For an end-to-end check, use the current Sub2API origin without a path and a local ClinePass group key; the monitor appends `/chat/completions`. Each check performs a real generation and consumes ClinePass quota.
