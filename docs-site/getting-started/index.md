...
完成下面四步，即可接入 OnprsCodexApi。全程只需要控制台账号、一个 API Key、可用模型名和正确的协议端点。

## 1. 登录控制台

打开 [OnprsCodexApi 控制台](https://api.onprs.top/)。尚未注册时，按页面当前开放的注册方式完成账号创建；注册方式和验证要求可能随运营配置变化。

登录后先进入[个人中心](https://api.onprs.top/profile)，确认邮箱和账号状态正常。不要与他人共用登录密码。

## 2. 创建 API Key

进入 [API Keys](https://api.onprs.top/keys)，为当前设备或应用创建独立 Key。名称建议包含用途和环境，例如 `codex-laptop` 或 `team-app-prod`。
...
进入[可用渠道](https://api.onprs.top/available-channels)确认分组当前可用模型，再根据客户端选择协议：
...
- [用量记录](https://api.onprs.top/usage)稍后能看到对应请求、模型和费用。
