...
2. 在[可用渠道](https://api.onprs.top/available-channels)复制该 Key 分组可用的模型名。
...
主站 [API Keys](https://api.onprs.top/keys) 的“使用 Key”入口可生成适配当前 Key、分组和模型的配置。自动导入前应查看脚本内容；脚本会备份已有配置，但仍建议先自行备份。
