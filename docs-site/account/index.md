...
API Key 可以代表账号发起付费请求。把它按生产凭据管理，而不是普通配置字符串。
...
推荐使用系统凭据库、CI secret、受限环境变量或权限为 `0600` 的独立文件。禁止放入：
...
新建 Key、逐个切换客户端、观察用量、禁用旧 Key，最后撤销。不要先删除旧 Key再测试新 Key。
...
仅从最新提交删除密钥不够；密钥仍可能存在于 Git 历史和缓存中，必须轮换。
