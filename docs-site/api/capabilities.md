...
Responses、Anthropic 和 Google 能表达不同形式的 reasoning 或 thought signature。跨协议会在能力允许时保留；无法无损表达时可能返回能力错误。OpenCode Go 对 provider-only reasoning signature 有既定响应降级策略，但请求侧仍保持严格转换。
...
- 批量图片：控制台[批量图片](https://api.onprs.top/batch-image)及 `/v1/images/batches` 系列接口。
