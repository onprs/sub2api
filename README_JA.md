...
- `security.url_allowlist.allow_openai_apikey_custom_hosts` - OpenAI API キーアカウントが任意の公開 HTTPS 中継アドレスを使えるようにする（デフォルト true。false にすると base_url も upstream_hosts 許可リストの厳格な境界に従う）
- `security.url_allowlist.allow_anthropic_apikey_custom_hosts` - Anthropic API キーアカウントが任意の公開 HTTPS 中継アドレスを使えるようにする（デフォルト true。false にすると base_url も upstream_hosts 許可リストの厳格な境界に従う）
