...
function translatedUnit(t: TranslateFn, key: string, fallback: string): string {
  const label = t(key)
...
export function planValiditySuffix(
  ...
  if (base === 'month') {
    return days === 1
      ? translatedUnit(t, 'payment.perMonth', 'month')
      : `${days}${translatedUnit(t, 'payment.months', 'months')}`
  }
  if (base === 'week') {
    return `${days}${translatedUnit(t, 'payment.weeks', 'weeks')}`
  }
  // 其余单位（含数据库默认的 day 与未知值）后端一律按天计费，展示保持一致。
  return `${days}${translatedUnit(t, 'payment.days', 'days')}`
}
