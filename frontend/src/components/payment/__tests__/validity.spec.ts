...
describe('planValiditySuffix', () => {
  ...
  it('keeps user-readable units when a translation key is unavailable', () => {
    const identity = (key: string) => key
    expect(planValiditySuffix({ validity_days: 1, validity_unit: 'months' }, identity)).toBe('month')
    expect(planValiditySuffix({ validity_days: 3, validity_unit: 'months' }, identity)).toBe('3months')
    expect(planValiditySuffix({ validity_days: 2, validity_unit: 'weeks' }, identity)).toBe('2weeks')
    expect(planValiditySuffix({ validity_days: 30, validity_unit: 'days' }, identity)).toBe('30days')
  })
})
