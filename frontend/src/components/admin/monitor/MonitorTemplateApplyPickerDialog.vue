    ...
    <div v-else>
      ...
      <ul class="max-h-80 divide-y divide-gray-100 overflow-y-auto rounded-lg border border-gray-200 dark:divide-dark-700 dark:border-dark-700">
        ...
        >
          ...
          <span v-if="monitorSelectableAPIModes(m.provider).length > 1" class="text-xs text-gray-400">{{ m.api_mode }}</span>
          <span
            v-if="!m.enabled"
...
import { monitorSelectableAPIModes } from '@/constants/channelMonitor'
