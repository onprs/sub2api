...
describe('CreateAccountModal', () => {
  ...
  it('creates OpenCode Zen as an API key account with the default upstream base URL', async () => {
    ...
    expect(payload.credentials).toMatchObject({
      ...
      protocol_rules: expect.any(Array)
    })
  ...
  it('clears the API key when switching providers', async () => {
    const wrapper = mountModal()
    await selectButtonByText(wrapper, 'OpenRouter')
    await wrapper.get('[data-tour="account-form-name"]').setValue('Provider switch')
    const firstKeyInput = wrapper.findAll('input[type="password"]').find((input) =>
      (input.attributes('placeholder') || '').includes('sk-')
    )
    expect(firstKeyInput).toBeDefined()
    await firstKeyInput!.setValue('sk-provider-one')

    await selectButtonByText(wrapper, 'ClinePass')
    await flushPromises()

    const switchedKeyInput = wrapper.findAll('input[type="password"]').find((input) =>
      (input.attributes('placeholder') || '').includes('sk-')
    )
    expect((switchedKeyInput!.element as HTMLInputElement).value).toBe('')
    await wrapper.get('form#create-account-form').trigger('submit.prevent')
    await flushPromises()
...
describe('CreateAccountModal OpenAI long-context billing', () => {
  ...
  it('antigravity upstream 创建默认携带上游倍率探测开关', async () => {
    ...
    const baseInput = wrapper.get('[data-testid="antigravity-upstream-base-url"]')
