...
describe('BulkEditAccountModal', () => {
  ...
  it('OpenAI 账号批量编辑可开启 GPT-5.6 缓存写入推断并设置阈值', async () => {
    const wrapper = mountModal({
      selectedPlatforms: ['openai'],
      selectedTypes: ['oauth', 'apikey']
    })

    await wrapper.get('#bulk-edit-infer-gpt56-cache-write-enabled').setValue(true)
    await wrapper.get('[data-testid="bulk-edit-infer-gpt56-cache-write-toggle"]').trigger('click')
    await wrapper.get('[data-testid="bulk-edit-infer-gpt56-cache-write-min-tokens"]').setValue('2048')
    await wrapper.get('#bulk-edit-account-form').trigger('submit.prevent')
    await flushPromises()
    ...
    expect(adminAPI.accounts.bulkUpdate).toHaveBeenCalledWith([1, 2], {
      extra: {
        ...
        infer_gpt56_cache_write_min_tokens: 2048
      }
    })
  })

  it('OpenAI 账号批量编辑可明确关闭 GPT-5.6 缓存写入推断', async () => {
    const wrapper = mountModal({
      selectedPlatforms: ['openai'],
      selectedTypes: ['oauth']
    })
    ...
    expect(adminAPI.accounts.bulkUpdate).toHaveBeenCalledWith([1, 2], {
      extra: {
        ...
        infer_gpt56_cache_write_min_tokens: 0
      }
    })
  })

  it('筛选 OpenAI 账号批量编辑可提交 GPT-5.6 缓存写入推断', async () => {
    const wrapper = mountModal({
      accountIds: [],
      selectedPlatforms: [],
      selectedTypes: [],
