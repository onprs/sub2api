...
// ── 平台限额类型 ──────────────────────────────────────────────────
export type PlatformType = "anthropic" | "openai" | "gemini" | "antigravity" | "grok" | "opencode_go"
...
const PLATFORMS: PlatformType[] = ["anthropic", "openai", "gemini", "antigravity", "grok", "opencode_go"]

/** 归一化为全 6 平台 × 3 窗口（缺失填 null），供模板非空绑定 */
...
/** 提交前清洗：非有限数/负数/空字符串 → null（保留 0 = 显式禁用），返回全 6 平台嵌套 map */
