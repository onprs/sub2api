...
export type Provider = 'openai' | 'anthropic' | 'gemini' | 'grok' | 'antigravity_claude' | 'antigravity_gemini' | 'opencode_go' | 'clinepass' | 'openrouter' | 'commandcode'
...
export async function listOpenRouterModels(): Promise<string[]> {
  const { data } = await apiClient.get<{ models: string[] }>('/admin/channel-monitors/openrouter-models')
  return data.models || []
}
