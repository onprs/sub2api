...
import {
  ...
  buildAPIKeyBalanceRefreshKey,
  buildOpenAIUsageRefreshKey
} from '../accountUsageRefresh'

describe('buildAPIKeyBalanceRefreshKey', () => {
  it('OpenAI API Key 的 Base URL 或代理变化时会生成不同余额刷新 key', () => {
    ...
    expect(buildAPIKeyBalanceRefreshKey(base)).not.toBe(buildAPIKeyBalanceRefreshKey(next))
    expect(buildAccountUsageRefreshKey(base)).not.toBe(buildAccountUsageRefreshKey(next))
  })

  it('非 API Key 不生成余额刷新 key', () => {
    expect(buildAPIKeyBalanceRefreshKey({
      id: 9,
      platform: 'openai',
      type: 'oauth',
      updated_at: '2026-08-13T10:00:00Z',
  ...
  it('Grok API Key 的 Base URL 或代理变化时会生成不同余额刷新 key', () => {
    const base = {
      id: 10,
      platform: 'grok',
      type: 'apikey',
