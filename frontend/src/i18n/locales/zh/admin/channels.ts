export default {
    ...
    channels: {
      ...
      emptyModelsInPricing: '每条定价配置至少需要包含一个模型',
    ...
    channelMonitor: {
      ...
      form: {
        ...
        apiModeResponsesHint: '使用 /v1/responses，默认带 instructions + input；适合本站自检、Codex 与 OpenCode Go Responses 模型。',
        ...
        openRouterEndpointHint: '推荐使用当前服务的基础域名（不带路径）做端到端检测，并选择本站 OpenRouter 分组 Key。检测器会自动拼接 /chat/completions。',
        apiKey: 'API Key',
        apiKeyPlaceholder: '请输入 API Key',
        apiKeyEditPlaceholder: '留空表示不修改',
        clinePassAPIKeyHint: '当前服务地址使用本站分组 Key；Cline 官方地址必须使用官方签发的 Cline API Key，两者不能混用。',
        openRouterAPIKeyHint: '当前服务地址使用本站分组 Key；OpenRouter 官方地址必须使用官方签发的 API Key，两者不能混用。',
        useMyKey: '使用我的 Key',
        ...
        clinePassIntervalHint: '每次检测都会发起一次真实生成并消耗 ClinePass 额度，建议使用较长间隔。',
      ...
      advanced: {
        ...
        bodyModeHintMerge: '与默认请求体浅合并，用户字段优先；但 model / messages / instructions / input / contents 等路由字段会被保护（动这些字段请用「覆盖」模式）。',
    ...
    subscriptions: {
      ...
      restore: '恢复',
      resetQuota: '重置配额',
      resetSelectedQuota: '重置已选配额',
      resetFilteredQuota: '重置筛选结果配额',
      resetQuotaTitle: '重置用量配额',
      resetQuotaConfirm: "确定要重置 '{user}' 的所选滚动配额窗口吗？",
      resetQuotaSelectedConfirm: '确定要重置已选 {count} 个订阅的所选滚动配额窗口吗？',
      resetQuotaFilteredConfirm: '确定要重置当前筛选结果中的 {count} 个订阅的所选滚动配额窗口吗？',
      resetQuotaWindows: '选择要重置的窗口',
      resetQuotaHierarchyHint: '滚动窗口按层级处理。若同时选择多个滚动窗口，后端只执行最大的已选窗口，避免重复扣减。',
      quotaWindowFiveHour: '5 小时窗口',
      quotaWindowSevenDay: '7 天窗口',
      quotaWindowThirtyDay: '30 天窗口',
      resetFiveHourHint: '清零 5h 用量，并从 7d/30d 累计中扣减本次清零值。',
      resetSevenDayHint: '清零 7d 及其子窗口 5h 用量，并从 30d 累计中扣减本次清零值。',
      resetThirtyDayHint: '清零 30d 及其子窗口 7d/5h 用量。',
      selectQuotaWindow: '请至少选择一个配额窗口',
      selectedCount: '已选择 {count} 个订阅',
      selectCurrentPage: '选择当前页',
      noSelection: '请先选择订阅',
      quotaResetSuccess: '配额重置成功',
      quotaResetPartialSuccess: '部分配额重置成功：成功 {success}，失败 {failed}',
      quotaResetCacheWarning: '配额已重置成功，但缓存失效出现警告，数据可能需要稍后刷新。',
      ...
      guide: {
        ...
        actions: {
          ...
          resetQuota: '重置配额',
          resetQuotaDesc: '按层级清零滚动窗口：重置 7d 会同时清零 5h，重置 30d 会清零所有子窗口',
          revoke: '撤销',
