export default {
    accounts: {
      ...
      firstOutputFailoverTimeout: '首输出换号预算（秒）',
      firstOutputFailoverTimeoutPlaceholder: '留空表示不启用',
      firstOutputFailoverTimeoutHint: '仅此 OpenAI API Key 生效；在首个有效输出前超时会切换到下一账号。',
      firstOutputFailoverCooldown: '账号换号冷却（分钟）',
      firstOutputFailoverCooldownPlaceholder: '留空表示不立即冷却',
      firstOutputFailoverCooldownHint: '命中首输出预算或上游返回 502/503/504 后，冷却期间后续请求会直接跳过此账号。',
      expiresAt: '过期时间',
      ...
      upstream: {
        baseUrl: '上游 Base URL',
        baseUrlHint: '上游兼容网关地址，例如：https://gateway.example.com/antigravity',
        apiKey: '上游 API Key',
