export default {
  ...
  modelPricing: {
    ...
    help: {
      open: '查看计费说明',
      title: '模型计费说明',
      intro: '本页原始价格使用美元符号，实际价格使用人民币符号。Token 价格按每百万 Token 展示。模型峰谷时段或上下文档位先决定原始单价；实际价格再按所选分组与活动折算后的模型额度倍率换算。',
      priceModes: {
        title: '原始价格与实际价格',
        rawTitle: '原始价格',
      ...
      contextTiers: {
        title: '上下文档位',
        description: 'Token 请求通常按输入侧上下文总量匹配档位，即输入 Token、缓存写入 Token 与缓存读取 Token 之和。命中档位后，该档位的输入、输出和缓存单价作为原始价格。',
        boundary: '“不超过 256K”包含上限；“超过 256K”从该边界之后开始。若显示自定义档位名称，则以该名称对应的配置规则为准。',
  ...
  payment: {
    ...
    admin: {
      ...
      auditLogs: '操作日志',
      auditActions: {
        AFFILIATE_REBATE_APPLIED: '邀请返利已发放',
        AFFILIATE_REBATE_FAILED: '邀请返利发放失败',
        AFFILIATE_REBATE_SKIPPED: '邀请返利已跳过',
        FULFILLMENT_FAILED: '订单履约失败',
        ORDER_CANCELLED: '订单已取消',
        ORDER_CREATED: '订单已创建',
        ORDER_EXPIRED: '订单已过期',
        ORDER_PAID: '订单已支付',
        ORDER_RECOVERED: '订单已恢复',
        PAYMENT_AFTER_EXPIRY: '订单过期后收到付款',
        PAYMENT_AMOUNT_MISMATCH: '支付金额不匹配',
        PAYMENT_INVALID_AMOUNT: '支付金额无效',
        PAYMENT_PROVIDER_METADATA_MISMATCH: '支付渠道元数据不匹配',
        PAYMENT_PROVIDER_MISMATCH: '支付渠道不匹配',
        RECHARGE_RETRY: '充值已重试',
        RECHARGE_SUCCESS: '充值成功',
        REFUND_FAILED: '退款失败',
        REFUND_GATEWAY_FAILED: '退款网关处理失败',
        REFUND_NO_TRADE_NO: '退款缺少交易号',
        REFUND_PENDING: '退款处理中',
        REFUND_PROVIDER_METADATA_MISMATCH: '退款渠道元数据不匹配',
        REFUND_QUERY_PENDING: '退款状态查询中',
        REFUND_REQUESTED: '已申请退款',
        REFUND_ROLLBACK_FAILED: '退款回滚失败',
        REFUND_SUCCESS: '退款成功',
        SUBSCRIPTION_ASSIGNED: '订阅已分配',
        SUBSCRIPTION_SUCCESS: '订阅已发放'
      },
      operator: '操作人',
      channelName: '渠道名称',
      ...
      stock: '库存',
      stockHint: '留空 = 不限库存，0 = 售罄。',
      stockInvalid: '库存不能为负数',
      unlimited: '无限制',
