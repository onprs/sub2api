      ...
      <template #table>
        ...
        >
          ...
          <template #cell-group="{ row }">
            <button
              @click="openRoutingEditor(row)"
              class="-mx-2 -my-1 flex min-h-8 max-w-64 items-center gap-2 rounded-lg px-2 py-1 text-left transition-colors hover:bg-gray-100 dark:hover:bg-dark-700"
              :title="t('keys.routing.edit')"
            >
              <GroupBadge
                v-if="routingPrimaryGroup(row)"
                :name="routingPrimaryGroup(row)!.name"
                :platform="routingPrimaryGroup(row)!.platform"
                :subscription-type="routingPrimaryGroup(row)!.subscription_type"
                :rate-multiplier="routingPrimaryGroup(row)!.rate_multiplier"
                :dynamic-rate-enabled="routingPrimaryGroup(row)!.dynamic_rate_enabled"
                :dynamic-rate-min-multiplier="routingPrimaryGroup(row)!.dynamic_rate_min_multiplier"
                :dynamic-rate-max-multiplier="routingPrimaryGroup(row)!.dynamic_rate_max_multiplier"
                :user-rate-multiplier="userGroupRates[routingPrimaryGroup(row)!.id]"
                :peak-rate-enabled="routingPrimaryGroup(row)!.peak_rate_enabled"
                :peak-start="routingPrimaryGroup(row)!.peak_start"
                :peak-end="routingPrimaryGroup(row)!.peak_end"
                :peak-rate-multiplier="routingPrimaryGroup(row)!.peak_rate_multiplier"
                class="min-w-0"
              />
              <span v-else class="text-sm text-gray-400 dark:text-dark-500">{{ t('keys.noGroup') }}</span>
              <span
                v-if="routingGroupCount(row) > 1"
                class="inline-flex h-5 min-w-5 shrink-0 items-center justify-center rounded bg-gray-100 px-1.5 text-xs font-semibold tabular-nums text-gray-600 dark:bg-dark-700 dark:text-gray-300"
              >
                +{{ routingGroupCount(row) - 1 }}
              </span>
              <Icon name="cog" size="sm" class="shrink-0 text-gray-400" />
            </button>
          </template>
    ...
    >
      ...
      <template #footer>
        <div class="flex justify-end gap-3">
          ...
          >
            {{ routingSubmitting ? t('keys.saving') : t('common.update') }}
          </button>
        </div>
      </template>
    </BaseDialog>
...
import {
  ...
  createRoutingInput as routingInputFromDraft,
  getRoutingGroupCount as routingGroupCount,
  getRoutingPrimaryGroup as routingPrimaryGroup
...
const routingEditorDraft = ref<ApiKeyRoutingDraft>(emptyRoutingDraft())
...
const closeRoutingEditor = () => {
  if (routingSubmitting.value) return
  showRoutingEditor.value = false
  routingEditorKey.value = null
  routingEditorDraft.value = emptyRoutingDraft()
}

const saveRoutingEditor = async () => {
  const key = routingEditorKey.value
  const routing = routingInputFromDraft(routingEditorDraft.value)
  if (!key || !routing) {
  ...
  try {
    const updated = await keysAPI.updateRouting(key.id, routing)
    ...
    appStore.showError(error.response?.data?.detail || error.message || t('keys.routing.updateFailed'))
  } finally {
