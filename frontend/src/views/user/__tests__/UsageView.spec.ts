...
describe('user UsageView', () => {
  ...
  it('includes API keys after the first page in the record filter and queries by the selected key', async () => {
  ...
  it('keeps the initial filters, sort, and filename while exporting multiple pages', async () => {
    ...
    queryRequests.mockClear()
    queryRequests.mockImplementation((params, options) =>
    ...
    try {
      ...
      const initialParams = { ...queryRequests.mock.calls[0][0] }
      ...
      const exportCalls = queryRequests.mock.calls.filter((call) => call.length === 1)
