...
const exportToCSV = async () => {
  ...
  try {
    ...
    for (let page = 1; page <= totalPages; page++) {
      const response = await usageAPI.queryRequests({ ...exportParams, page })
    ...
    const rows = allLogs.map((log) => [
      ...
      log.upstream_model_mismatch == null ? '' : String(log.upstream_model_mismatch),
      ...
      isErrorRequest(log) ? '' : (log.dynamic_rate_multiplier ?? ''),
