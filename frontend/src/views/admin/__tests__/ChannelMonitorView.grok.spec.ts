...
async function selectExternalTarget(wrapper: ReturnType<typeof mountDialog>) {
  const externalButton = wrapper
    .findAll('button')
    .find((button) => button.text() === 'admin.channelMonitor.form.targetExternal')
...
describe('channel monitor Grok provider', () => {
  ...
  it('offers Grok in the responsive provider grid and prefills its official defaults', async () => {
    ...
    await selectExternalTarget(wrapper)
