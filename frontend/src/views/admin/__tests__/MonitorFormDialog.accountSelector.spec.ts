...
describe('MonitorFormDialog linked account selector', () => {
  ...
  it('keeps a picked account visible when a later search hides it', async () => {
    ...
    const payload = monitorCreate.mock.calls[0][0]
  ...
  it('uses the account platform behind an Antigravity monitor variant', async () => {
    const wrapper = mountDialog()
    await wrapper.get('[data-testid="monitor-provider-antigravity_claude"]').trigger('click')
    await wrapper.get('[data-testid="monitor-check-mode-quota"]').trigger('click')
    await flushPromises()

    expect(accountsList).toHaveBeenLastCalledWith(
      1,
      50,
  ...
  it('sends account_id zero when an edited quota monitor switches back to probe', async () => {
    accountsList.mockResolvedValue({
      items: [{ id: 1, name: 'linked account', platform: 'anthropic' }],
    })
    const wrapper = mountDialog(
      makeMonitor({
        check_mode: 'quota',
  ...
  it('restores probe mode and clears the quota placeholder when switching away from antigravity', async () => {
    ...
    const primaryModel = wrapper.get('[data-testid="monitor-primary-model"]')
    expect((primaryModel.element as HTMLInputElement).value).toBe('')
    ...
    await primaryModel.setValue('claude-sonnet-4-5')
    await wrapper.get('[data-testid="monitor-endpoint"]').setValue('https://api.anthropic.com')
    await wrapper.get('input[type="password"]').setValue('new-anthropic-key')
    await wrapper.get('#channel-monitor-form').trigger('submit')
    await flushPromises()
  ...
  it('keeps account_id null when creating a probe monitor', async () => {
    ...
    await wrapper.findAll('input[type="text"]')[0].setValue('my monitor')
    await wrapper.get('[data-testid="monitor-primary-model"]').setValue('claude-sonnet-4-5')
    await wrapper.get('[data-testid="monitor-target-external"]').trigger('click')
    await wrapper.get('[data-testid="monitor-endpoint"]').setValue('https://api.anthropic.com')
    await wrapper.get('input[type="password"]').setValue('new-anthropic-key')
    await wrapper.get('#channel-monitor-form').trigger('submit')
    await flushPromises()
