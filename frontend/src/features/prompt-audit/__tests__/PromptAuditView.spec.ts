...
const EndpointStub = defineComponent({
  ...
  template: '<div data-test="endpoint"><button data-test="inject-secret" @click="$emit(\'update:endpoints\', endpoints.map((e) => ({ ...e, token: \'prompt-audit-test-token\' })))">secret</button><button data-test="probe" @click="$emit(\'probe\', endpoints[0])">probe</button></div>',
})
...
describe('PromptAuditView', () => {
  ...
  it('clears plaintext token state after a successful save', async () => {
    ...
    await flushPromises()
    expect(mocks.updateConfig).toHaveBeenCalledWith(expect.objectContaining({ endpoints: [expect.objectContaining({ token: 'prompt-audit-test-token' })] }))
    ...
    expect(wrapper.html()).not.toContain('prompt-audit-test-token')
  })
