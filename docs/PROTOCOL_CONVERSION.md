...
- Anthropic API-key native passthrough creates a fresh same-protocol Pipeline for each actual upstream attempt after beta/body sanitization. Buffered JSON success crosses `transport.Response` through that attempt's Pipeline and the Anthropic renderer while preserving exact response bytes and unknown fields. Its streaming success loop still retains the legacy scanner until timeout, keepalive, partial-record protection, and disconnect draining are moved together to a bounded structured stream;
