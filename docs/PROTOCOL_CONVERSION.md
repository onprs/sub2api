...
- Gemini compatibility Chat Completions, OpenAI Responses, and Anthropic Messages request/response/stream conversion to Google GenAI through one shared provider executor. Their Pipelines use the scoped provider metadata bridge to preserve real Google `thoughtSignature` values across tool-call turns for both buffered and streaming responses; tenant/API-key/group/account/protocol isolation prevents account failover or unrelated callers from reusing them;
