...
- Native Gemini `generateContent` creates a fresh Google-to-Google identity Pipeline for every actual generation attempt after empty-part filtering and thought-signature provider policy, but before Code Assist wrapping. AI Studio and Vertex unary successes, plus Code Assist SSE-as-unary aggregation, cross `transport.Response` with explicit Google actual protocol and return through the successful attempt's Pipeline and the Google renderer. Code Assist raw envelopes/stream bytes remain structured metadata; retries, auth, sticky signatures, usage, billing, and `countTokens` fallback remain service-owned. Native `streamGenerateContent` still retains its characterized line reader and is the next migration boundary;
