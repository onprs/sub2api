...
- Native Gemini generation creates a fresh Google-to-Google identity Pipeline for every actual HTTP attempt after empty-part filtering and thought-signature provider policy, but before Code Assist wrapping. AI Studio and Vertex unary successes, plus Code Assist SSE-as-unary aggregation, cross `transport.Response`; native `streamGenerateContent` crosses a bounded `transport.Stream`, the successful attempt's identity `StreamSession`, and the Google renderer. Code Assist request/response envelopes stay outside standard conversion, with raw transformed records retained as bounded transport metadata. Native streams preserve EOF and upstream `[DONE]` as compatible terminals, delay downstream commitment until the first valid Google event, and continue draining usage after client disconnect. Retries, auth, sticky signatures, usage, billing, and `countTokens` fallback remain service-owned;
