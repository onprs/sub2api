...
- Anthropic-platform Chat Completions, Responses, and Google GenAI request conversion, complete responses, and streaming responses for direct Anthropic-compatible and Vertex accounts; their always-streaming Anthropic attempts expose immediate HTTP errors through `transport.Stream.ErrorBody`, detach an actual-Anthropic `transport.Response`, and apply source callback or failover policy before downstream commitment. Google ingress shares the OpenAI/Anthropic account loop while retaining each provider's auth, health, usage, and billing ownership. AWS Bedrock native Messages uses its SigV4/API-key and binary EventStream adapter; cross-protocol Chat, Responses, and Google ingress still require that adapter to be connected after standard conversion rather than entering the direct-Anthropic HTTP executor;
