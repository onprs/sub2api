...
管理员可以在 **兑换码管理 → 生成兑换码 → 订阅 → 按套餐** 中选择一个订阅套餐并批量生成兑换码。用户兑换后会获得与购买该套餐一致的订阅分组、有效期，以及 `5h / 7d / 30d` 滚动限额。

按套餐生成时，系统会把套餐当时的权益快照写入兑换码：
...
已生成的套餐兑换码按“生成时的套餐权益”生效。后续管理员修改套餐价格、有效期或滚动限额，不会影响已经生成出去的兑换码。

旧的 **按分组** 生成方式仍然保留，适合只赠送某个订阅分组和固定天数。按分组生成的旧式订阅兑换码不会刷新用户已有套餐订阅的滚动限额；按套餐生成的兑换码会在兑换或续期时刷新为该兑换码里的套餐限额快照。
